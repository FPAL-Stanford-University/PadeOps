module SolidMixtureMod
	...
	logical :: use_gradVF, energy_surfTen  !flag to use VF in calculating surface tension
        ...
        logical :: skew_mass = .false.,skew_VF = .false.,skew_Ys = .false.
...
contains
    ...
    subroutine init(this,decomp,der,derD02,derStagg,derStaggd02,derD06,derCD06,derD04,interpMid,interpMid02,interpMid04,use_Stagg,LADN2F,LADInt,LADMass_Consv,fil,gfil,LAD,ns,PTeqb,pEqb,pRelax,SOSmodel,use_gTg,updateEtot,useAkshayForm,twoPhaseLAD,LAD5eqn,useOneG,intSharp,usePhiForm,intSharp_cpl,intSharp_cpg,intSharp_cpg_west,intSharp_spf,intSharp_ufv,intSharp_utw,intSharp_d02,intSharp_msk,intSharp_flt,intSharp_gam,intSharp_eps,intSharp_cut,intSharp_dif,intSharp_tnh,intSharp_pfloor,use_surfaceTension,use_normFV,use_normInt,use_gradXi, energy_surfTen,use_gradphi, use_gradVF, surfaceTension_coeff, use_FV,use_XiLS, XiLS_eps,use_D04,surface_mask, weightedcurvature, strainHard,cnsrv_g,cnsrv_gt,cnsrv_gp,cnsrv_pe,x_bc,y_bc,z_bc,SpongeLayer,skew_mass,skew_Ys,skew_VF)
        ...
        logical,                         intent(in)    :: use_surfaceTension, use_gradphi,use_Stagg, use_gradVF, use_FV, use_D04,surface_mask, weightedcurvature, use_gradXi, energy_surfTen, use_XiLS,LADN2F,LADInt,use_normFV, use_normInt, LADMass_Consv, skew_mass, skew_VF, skew_Ys
	...
	this%use_gradVF 	  = use_gradVF  
     ...
     do i=1,this%ns
        ...
        deallocate(dummy)
         ...
         print *, " mix skew mass ", this%skew_mass
         print *, " mix kew Ys   ", this%skew_Ys
         print *, " mix skew VF   ", this%skew_VF
       ...
       if(allocated(this%surfaceTension_fxy_y)) deallocate(this%surfaceTension_fxy_y)
        ...
        allocate(this%J_phi(this%nxp, this%nyp, this%nzp, 3,2))

        if(allocated(this%J_VF)) deallocate(this%J_VF)
        allocate(this%J_VF(this%nxp, this%nyp, this%nzp, 3,2))
        ...
        allocate(this%tag(this%nxp, this%nyp, this%nzp))
...
!           stop
    ...
    subroutine getLAD(this,rho,p,e,u,v,w,duidxj,sos,detady,dy_stretch,use_gTg,strainHard,periodicx,periodicy,periodicz,x_bc,y_bc,z_bc,tfloor,dt)
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp)   :: umag,rhom1,rhom2,c1,c2,cVF,rhom,Ys_fil,VF_fil
...
!                 enddo
      ...
      enddo
       ...
       tmpYs = 0.0
       do i = 1,this%ns
            call gradFV_N2Fx(this%decomp,this%derStagg,rho*this%material(i)%Ys,dYdx_x(:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
            call gradFV_N2Fy(this%decomp,this%derStagg,rho*this%material(i)%Ys,dYdy_y(:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
            call gradFV_N2Fz(this%decomp,this%derStagg,rho*this%material(i)%Ys,dYdz_z(:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
            call interpolateFV(this%decomp,this%interpMid,this%material(i)%rhodiff,rhodiff_int(:,:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
            call interpolateFV(this%decomp,this%interpMid,this%material(i)%VF,VF_int(:,:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
            call this%material(i)%getSpeciesDensity(rho,rhom)
            call interpolateFV(this%decomp,this%interpMid,rhom,rhom_int(:,:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)

            tmpYs(:,:,:,1) = tmpYs(:,:,:,1) + rhodiff_int(:,:,:,1,i)*dYdx_x(:,:,:,i)
            tmpYs(:,:,:,2) = tmpYs(:,:,:,2) + rhodiff_int(:,:,:,2,i)*dYdy_y(:,:,:,i)
            tmpYs(:,:,:,3) = tmpYs(:,:,:,3) + rhodiff_int(:,:,:,3,i)*dYdz_z(:,:,:,i)

             
         enddo

         do i = 1,this%ns
            this%material(i)%JiYs = VF_int(:,:,:,:,i)*tmpYs
         enddo

      if(this%LADMass_Consv) then
         ...
         do i = 1,this%ns
            ...
            call this%material(i)%getSpeciesDensity(rho,rhom)
            call interpolateFV(this%decomp,this%interpMid,rhom,rhom_int(:,:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
         !   call gradFV_N2Fx(this%decomp,this%derStagg,this%material(i)%VF,dYdx_x(:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
         !   call gradFV_N2Fy(this%decomp,this%derStagg,this%material(i)%VF,dYdy_y(:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
         !   call gradFV_N2Fz(this%decomp,this%derStagg,this%material(i)%VF,dYdz_z(:,:,:,i),periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)

         enddo

        this%J_phi(:,:,:,1,1) = rhodiff_int(:,:,:,1,1)*dYdx_x(:,:,:,1) - VF_int(:,:,:,1,1)*(rhodiff_int(:,:,:,1,1)*dYdx_x(:,:,:,1) &
                                                                                     + rhodiff_int(:,:,:,1,2)*dYdx_x(:,:,:,2))

        this%J_phi(:,:,:,1,2) = rhodiff_int(:,:,:,1,2)*dYdx_x(:,:,:,2) - VF_int(:,:,:,1,2)*(rhodiff_int(:,:,:,1,1)*dYdx_x(:,:,:,1) &
                                                                                     +rhodiff_int(:,:,:,1,2)*dYdx_x(:,:,:,2))

        this%J_phi(:,:,:,2,1) = rhodiff_int(:,:,:,2,1)*dYdy_y(:,:,:,1) - VF_int(:,:,:,2,1)*(rhodiff_int(:,:,:,2,1)*dYdy_y(:,:,:,1) &
                                                                                     +rhodiff_int(:,:,:,2,2)*dYdy_y(:,:,:,2))
             
        this%J_phi(:,:,:,2,2) = rhodiff_int(:,:,:,2,2)*dYdy_y(:,:,:,2) - VF_int(:,:,:,2,2)*(rhodiff_int(:,:,:,2,1)*dYdy_y(:,:,:,1) &
                                                                                     +rhodiff_int(:,:,:,2,2)*dYdy_y(:,:,:,2)) 

        this%J_phi(:,:,:,3,1) = rhodiff_int(:,:,:,3,1)*dYdz_z(:,:,:,1) - VF_int(:,:,:,3,1)*(rhodiff_int(:,:,:,3,1)*dYdz_z(:,:,:,1) &
                                                                                    +rhodiff_int(:,:,:,3,2)*dYdz_z(:,:,:,2))

        this%J_phi(:,:,:,3,2) = rhodiff_int(:,:,:,3,2)*dYdz_z(:,:,:,2) - VF_int(:,:,:,3,2)*(rhodiff_int(:,:,:,3,1)*dYdz_z(:,:,:,1) &
                                                                                    + rhodiff_int(:,:,:,3,2)*dYdz_z(:,:,:,2)) 

        this%J_VF(:,:,:,1,1) = adiff_int(:,:,:,1,1)*dYdx_x(:,:,:,1)/rhom_int(:,:,:,1,1) - VF_int(:,:,:,1,1)*( adiff_int(:,:,:,1,1)*dYdx_x(:,:,:,1)/rhom_int(:,:,:,1,1) &
                                                                                     + adiff_int(:,:,:,1,2)*dYdx_x(:,:,:,2)/rhom_int(:,:,:,1,2) )

        this%J_VF(:,:,:,1,2) = adiff_int(:,:,:,1,2)*dYdx_x(:,:,:,2)/rhom_int(:,:,:,1,2) - VF_int(:,:,:,1,2)*(adiff_int(:,:,:,1,1)*dYdx_x(:,:,:,1)/rhom_int(:,:,:,1,1) &
                                                                                     +adiff_int(:,:,:,1,2)*dYdx_x(:,:,:,2)/rhom_int(:,:,:,1,2) )

        this%J_VF(:,:,:,2,1) = adiff_int(:,:,:,2,1)*dYdy_y(:,:,:,1)/rhom_int(:,:,:,2,1) - VF_int(:,:,:,2,1)*(adiff_int(:,:,:,2,1)*dYdy_y(:,:,:,1)/rhom_int(:,:,:,2,1) &
                                                                                     +adiff_int(:,:,:,2,2)*dYdy_y(:,:,:,2)/rhom_int(:,:,:,2,2))

        this%J_VF(:,:,:,2,2) = adiff_int(:,:,:,2,2)*dYdy_y(:,:,:,2)/rhom_int(:,:,:,2,2) - VF_int(:,:,:,2,2)*(adiff_int(:,:,:,2,1)*dYdy_y(:,:,:,1)/rhom_int(:,:,:,2,1) &
                                                                                     +adiff_int(:,:,:,2,2)*dYdy_y(:,:,:,2)/rhom_int(:,:,:,2,2) )

        this%J_VF(:,:,:,3,1) = adiff_int(:,:,:,3,1)*dYdz_z(:,:,:,1)/rhom_int(:,:,:,3,1) - VF_int(:,:,:,3,1)*(adiff_int(:,:,:,3,1)*dYdz_z(:,:,:,1)/rhom_int(:,:,:,3,1) &
                                                                                    +adiff_int(:,:,:,3,2)*dYdz_z(:,:,:,2)/rhom_int(:,:,:,3,2) )

        this%J_VF(:,:,:,3,2) = adiff_int(:,:,:,3,2)*dYdz_z(:,:,:,2)/rhom_int(:,:,:,3,2) - VF_int(:,:,:,3,2)*(adiff_int(:,:,:,3,1)*dYdz_z(:,:,:,1)/rhom_int(:,:,:,3,1) &
                                                                                    + adiff_int(:,:,:,3,2)*dYdz_z(:,:,:,2)/rhom_int(:,:,:,3,2))
         ...
         do i = 1, this%ns
            ...
            call divergenceFV(this%decomp,this%derStagg,this%J_phi(:,:,:,1,i),this%J_phi(:,:,:,2,i),this%J_phi(:,:,:,3,i),this%material(i)%YsLAD,periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)

            call divergenceFV(this%decomp,this%derStagg,this%J_VF(:,:,:,1,i),this%J_VF(:,:,:,2,i),this%J_VF(:,:,:,3,i),this%material(i)%vfLAD,periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
    ...
    subroutine get_q(this,x_bc,y_bc,z_bc)
        ...
        do i = 1,this%ns
            if(.NOT. this%PTeqb) then
             ...
             !   print *, "material kap ", this%material(i)%kap
             !   print *, " tmp1_in_y ", tmp1_in_y
             !   print *, " Ys*Eh ", this%material(i)%Ys*this%material(i)%eh
                ! Step 3: Get qz
...
!         enddo
    ...
    subroutine get_intSharp_clean2(this,rho,ke_mid,x_bc,y_bc,z_bc,dx,dy,dz,periodicx,periodicy,periodicz,u,v,w,p)
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp,this%ns) :: rhoi,VFbound,hi,spf_a,spf_r
        real(rkind), dimension(this%nxp,this%nyp,this%nzp,3) :: phiint,gradxi,VFint,uFVint,vFVint,wFVint,gFVint,gtFVint,gpFVint,rhoFVint, kerne,gradFVFV, gradFV_N2F,rhoYsint,OOB_mask_int
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp) :: tmp,VF_fil,antiDiff,mask,RhoYsbound,filt,antiDiffFV,fmask,tanhmask,mask2,maskDiff,spf_f,spf_h,GVFmag,GVFmagT,antiDiffT,rhom, Db,H,OOB_mask,Hl,Hh
...
!              call interpolateFV_6(this,rho,rhoFVint_6,periodicx,periodicy,periodicz,this%x_bc,this%y_bc,this%z_bc)
            ...
            !  where( this%material(i)%rhodiff .ge. 1d-9 )
            !        H = 0
            !  elsewhere
            !        H = 1
            !  endwhere
...
!             if(this%intSharp_msk) then
!                where(( (this%material(i)%Ys .lt. cut_off) .OR. (this%material(i)%VF .lt. cut_off)  ) .AND. (abs(this%kappa*4*max(abs(this%material(i)%Ys*(1-this%material(i)%Ys)), abs(this%material(i)%VF*(1-this%material(i)%VF))) ) .lt. 0.075))
!                  antiDiffFVint(:,:,:,1,i) = zero
!                elsewhere(( (this%material(i)%Ys .gt. one-cut_off) .OR. (this%material(i)%VF .gt. one-cut_off)   ) .AND. (abs(this%kappa*4*max(abs(this%material(i)%Ys*(1-this%material(i)%Ys)),abs(this%material(i)%VF*(1-this%material(i)%VF))) ) .lt. 0.075 ) )
!                   antiDiffFVint(:,:,:,1,i) = zero
!                endwhere
!                where(((this%material(i)%Ys .lt. cut_off) .OR. (this%material(i)%VF .lt. cut_off)  ) .AND. (abs(this%kappa*4*max(abs(this%material(i)%Ys*(1-this%material(i)%Ys)), abs(this%material(i)%VF*(1-this%material(i)%VF))) ) .lt. 0.075 ) )
!                   antiDiffFVint(:,:,:,2,i) = zero
...
!             call interpolateFV(this,OOB_mask,OOB_mask_int,periodicx,periodicy,periodicz,-this%x_bc,this%y_bc,this%z_bc)

!             antiDiffFVint(:,:,:,1,i) = (1_rkind-OOB_mask_int(:,:,:,1))*antiDiffFVint(:,:,:,1,i)
!             antiDiffFVint(:,:,:,2,i) = (1_rkind-OOB_mask_int(:,:,:,2))*antiDiffFVint(:,:,:,2,i)
!             antiDiffFVint(:,:,:,3,i) = (1_rkind-OOB_mask_int(:,:,:,3))*antiDiffFVint(:,:,:,3,i)
...
!              call divergenceFV(this,rhoantiDiffFVint(:,:,:,:,i),this%material(i)%intSharp_RFV,dx,dy,dz,periodicx,periodicy,periodicz,this%x_bc,this%y_bc,this%z_bc)
            ...
            enddo
              ...
              !call interpolateFV_6(this,0.5*(u**2 + v**2 + w**2),keFV_int,periodicx,periodicy,periodicz,this%x_bc,this%y_bc,this%z_bc) 
              fv_h = zero
              fv_k = zero
              do i=1,this%ns
