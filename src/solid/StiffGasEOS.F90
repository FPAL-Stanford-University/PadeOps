...
contains
    ...
    pure subroutine get_entropy(this,T,p,s)
        class(stiffgas), intent(in) :: this
        real(rkind), dimension(:,:,:), intent(in)  :: T, p
        real(rkind), dimension(:,:,:), intent(out) :: s

        s = this%Cv*log(T**this%gam /( p + this%PInf)**(this%gam-one) )
