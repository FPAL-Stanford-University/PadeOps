module SolidGrid
	...
	logical     :: use_gradVF           !flag to use VF formulation in surface tension calculation		
        ...
        real(rkind) :: phys_mu1, phys_mu2,CP
...
contains
    subroutine init(this, inputfile )
        ...
        namelist /SINPUT/  gam, Rgas, PInf, shmod, &
                           ...
                           surfaceTension_coeff, R, p_amb, use_XiLS,XiLS_eps,LADInt, LADN2F, LADMass_Consv, Stretch1D, Stretch1Dx, Stretch1Dy, Stretch1Dz, SpongeLayer, useEigenFunction, alpha_skew, skew_mass, skew_Ys, skew_VF,CP
	...
	this%use_gradVF 	  = use_gradVF
        ...
        this%CP                   = CP
         print *, " sgrid skew mass ", this%skew_mass
         print *, " sgrid skew Ys   ", this%skew_Ys
         print *, " sgrid skew VF   ", this%skew_VF
 ...
 ! Initialize derivatives 
        ...
        call this%LAD%init(this%decomp,this%der,this%derStagg, this%interpMid,this%gfil,2,this%dx,this%dy,this%dz,Cbeta,CbetaP,Cmu,Ckap,CkapP,Cdiff,CY,Cdiff_g,Cdiff_gt,Cdiff_gp,Cdiff_pe,Cdiff_pe_2,Crho,Cvf1, Cvf2,Ce,Stretch1Dy)
...
!            do i = 1,3
!                call this%filter(this%pu_mid(:,:,:,i), this%fil, 1,-this%x_bc,this%y_bc,this%z_bc)
!               call this%filter(this%v_mid(:,:,:,i), this%fil, 1,this%x_bc,-this%y_bc,this%z_bc)
...
!        call this%FilDiffHeuristic()  
    ...
    subroutine getFaces( this)
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp,3) :: T1_int,T2_int,rhom_int,rhom2_int,rhoe1_int,rhoe2_int,rhoe_int,fluxu_int,fluxv_int,fluxw_int,spec_int,pgam,T_int,rhoc_int,rhocp_int,mu_int,mv_int,mw_int
...
!        call interpolateFV(this%decomp,this%interpMid,this%sos*this%rho,rhoc_int,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        ...
        call interpolateFV(this%decomp,this%interpMid,this%rho*this%u,fluxu_int,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)                
        call interpolateFV(this%decomp,this%interpMid,this%rho*this%v,fluxv_int,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
      ...
      do i = 1,2
        ...
        fluxu_int = fluxu_int + mu_int
        fluxv_int = fluxv_int + mv_int
        fluxw_int = fluxw_int + mw_int
        this%rho_mid = this%rho_mid + this%mix%material(i)%rhoYs_mid

      end do 

!      if (nrank.eq.0) print*,fluxu_int(:,:,:,1)-this%u_mid(:,:,:,1)
!      if (nrank.eq.0) print*,fluxv_int(:,:,:,2)-this%v_mid(:,:,:,2)

      this%u_int = fluxu_int(:,:,:,1) /this%rho_mid(:,:,:,1) - this%u_mid(:,:,:,1)
      this%v_int = fluxv_int(:,:,:,2)/this%rho_mid(:,:,:,2) - this%v_mid(:,:,:,2)

!      this%u_mid =  fluxu_int /this%rho_mid
!      this%v_mid =  fluxv_int/this%rho_mid
!      this%w_mid = 0
!      do i = 1,3
!       where( this%mix%material(1)%VF_mid(:,:,:,i) .LE. 1d-6 )
!             this%mix%material(1)%VF_mid(:,:,:,i) = 0.0
!             this%mix%material(1)%rhoYs_mid(:,:,:,i) = 0.0
!       elsewhere( this%mix%material(1)%VF_mid(:,:,:,i) .GE. 1-1d-6)

!             this%mix%material(1)%VF_mid(:,:,:,i) = 1.0 
!             this%mix%material(2)%rhoYs_mid(:,:,:,i) = 0.0 !M_int(:,:,:,2) = 0.0
...
!        call this%filter(this%mix%material(1)%VF_mid(:,:,:, i ), this%fil, 1,this%x_bc,this%y_bc, this%z_bc)
!        call this%filter(this%mix%material(1)%rhoYs_mid(:,:,:, i ), this%fil,1,this%x_bc,this%y_bc, this%z_bc)
!        call this%filter(this%mix%material(2)%rhoYs_mid(:,:,:, i ), this%fil,1,this%x_bc,this%y_bc, this%z_bc)
!        call this%filter(this%u_mid(:,:,:, i ),this%fil,1,this%x_bc,this%y_bc, this%z_bc)
!        call this%filter(this%v_mid(:,:,:, i ),this%fil,1,this%x_bc,this%y_bc,this%z_bc)
!        call this%filter(this%w_mid(:,:,:, i ),this%fil,1,this%x_bc,this%y_bc,this%z_bc)
!        call this%filter(this%p_mid(:,:,:, i ),this%fil,1,this%x_bc,this%y_bc,this%z_bc)

!       enddo
!       psi =  this%mix%intSharp_eps*log( ( (this%mix%material(1)%VF) + e )/ ((1 -  this%mix%material(1)%VF) + e) )
!        psi = (this%mix%material(1)%VF)**0.1_rkind / ( this%mix%material(1)%VF**0.1_rkind + max(eps,(1.0_rkind - this%mix%material(1)%VF))**0.1_rkind)
!       call interpolateFV(this%decomp,this%interpMid,psi,psi_int,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)


!       do i = 1,3
!       where(( this%mix%material(1)%VF .GE. 1D-3 ) .OR. ( this%mix%material(1)%VF .LE. (1_rkind - 1d-3 ) ) )

!          psi_safe = max(eps, min(1.0_rkind - eps, psi_int(:,:,:,i)))
!         this%mix%material(1)%VF_mid(:,:,:,i) = 0.5_rkind*(1.0_rkind + tanh(psi_int(:,:,:,i)/(2.0_rkind*this%mix%intSharp_eps ) ) )
!          this%mix%material(1)%VF_mid(:,:,:,i) = 1.0_rkind / ( 1.0_rkind + ((1.0_rkind - psi_safe )/(psi_safe) )**(1.0_rkind/0.1_rkind) )
!       endwhere
!       enddo

!       this%mix%material(1)%VF_mid =  this%mix%material(1)%VF_mid / (this%mix%material(1)%VF_mid +  this%mix%material(2)%VF_mid )
...
!          num = num + this%mix%material(i)%hydro%gam*this%mix%material(i)%hydro%Pinf*this%mix%material(i)%hydro%onebygam_m1*this%mix%material(i)%VF_mid
!          denom = denom + this%mix%material(i)%hydro%onebygam_m1*this%mix%material(i)%VF_mid 

!       enddo
  
!       this%p_mid = (rhoe_int - num ) / denom
!       this%rho_mid = this%mix%material(2)%rhoYs_mid / this%mix%material(2)%Ys_mid
!       this%mix%material(1)%rhoYs_mid = this%rho_mid * this%mix%material(1)%Ys_mid
!       rhom_int =  this%mix%material(2)%rhoYs_mid/this%mix%material(2)%Ys_mid 
!       this%mix%material(1)%rhoYs_mid = rhom_int*this%mix%material(1)%Ys_mid      
!       this%mix%material(2)%VF_mid = this%mix%material(2)%rhoYs_mid/this%mix%material(2)%rho_mid
!       this%mix%material(1)%VF_mid = 1-this%mix%material(2)%VF_mid
!       call this%mix%material(1)%getSpeciesDensity(this%rho,rhom)
!      call interpolateFV(this%decomp,this%interpMid04,this%mix%material(2)%T,T2_int,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc) 
!       rhom_int = this%mix%material(2)%rhoYs_mid /(this%mix%material(2)%VF_mid + 1d-15)
!       T2_int = (this%p_mid + this%mix%material(2)%hydro%Pinf) / (rhom_int * this%mix%material(2)%hydro%Cv * (this%mix%material(2)%hydro%gam - 1 ) )
!       T1_int = (T_int - this%mix%material(2)%VF_mid*T2_int) / this%mix%material(1)%VF_mid
       

!       this%mix%material(1)%rhoYs_mid = this%mix%material(1)%VF_mid*(this%p_mid + this%mix%material(1)%hydro%Pinf) &
!                                        / (T1_int * this%mix%material(1)%hydro%Cv * (this%mix%material(1)%hydro%gam - 1 ) )

!      this%mix%material(2)%rhoYs_mid = this%mix%material(2)%VF_mid*(this%p_mid + this%mix%material(2)%hydro%Pinf) & 
!                                       / (T2_int * this%mix%material(2)%hydro%Cv * (this%mix%material(2)%hydro%gam - 1 ) )

!       rhom_int = (this%p_mid + this%mix%material(1)%hydro%Pinf)/(this%mix%material(1)%hydro%Cv*e1_int*(this%mix%material(1)%hydro%gam-1))

!       this%mix%material(1)%rhoYs_mid = this%mix%material(1)%Ys_mid*this%rho_mid

!       call interpolateFV(this%decomp,this%interpMid,this%mix%material(2)%T,e1_int,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
!       rhom_int =  (this%p_mid + this%mix%material(2)%hydro%Pinf)/(this%mix%material(2)%hydro%Cv*e1_int*(this%mix%material(2)%hydro%gam-1))

!       this%mix%material(2)%rhoYs_mid = this%mix%material(2)%Ys_mid*this%rho_mid
...
!      call this%mix%material(2)%getSpeciesDensity(this%rho,rhom)
!      call interpolateFV(this%decomp,this%interpMid04,rhom,rhom_int,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)       
!      this%mix%material(2)%rhoYs_mid = this%mix%material(2)%VF_mid/rhom_int!*this%mix%material(2)%VF_mid 
!       this%mix%material(1)%VF_mid = 1.0 - this%mix%material(2)%VF_mid

 !      this%pError = this%p  - this%p_mid(:,:,:,2)
 !      this%u_int = this%u_mid(:,:,:,2)
 !      this%p_int = this%p_mid(:,:,:,2)
...
!print *, 'SOS: ', this%sos(179,1,1)
    ...
    subroutine getRHS(this, rhs, divu, viscwork)
        use decomp_2d, only: transpose_y_to_x, transpose_x_to_y,transpose_y_to_z, transpose_z_to_y
        use operators, only: divergence,gradient,divergenceFV, interpolateFV, interpolateFV_x, interpolateFV_y, interpolateFV_z, gradFV_N2Fx, gradFV_N2Fy, gradFV_N2Fz,interpolateMax
        use exits,      only: message,nancheck,GracefulExit
        class(sgrid), target, intent(inout) :: this
        ...
        if( .NOT. this%twoPhaseLAD) then
          ...
          if( this%LADInt .OR. this%LADN2F) then
           ...
           ! ke_int = half*(u_int**2 + v_int**2 + w_int**2)
             ...
             call divergenceFV(this%decomp,this%derStagg,this%u_mid(:,:,:,1)*Frho(:,:,:,1),this%u_mid(:,:,:,2)*Frho(:,:,:,2),this%u_mid(:,:,:,2)*Frho(:,:,:,3),this%uJ,this%periodicx, this%periodicy, this%periodicz,this%x_bc,this%y_bc,this%z_bc)
...
!print '(a,9(e21.14,1x))', 'tauxx', tauxx(89,1,1)
    ...
    subroutine getRHS_xStagg( this,  rhs, tauxx,tauxy,tauxz, qx)
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp) :: buff, flux,TE, u_int,v_int, w_int, p_int, tauxx_int, tauxy_int,tauxz_int, qx_int, e_int, rho_int, rhodiff_int, gam, num, t_int, KE,e_prim,p4, Eint,gradu,gradp,gradup, UU, kef,gradm1,gradm2,gradrhou,clocal, delp,gradVFx,gradVFy, gradVF,UV,umag,soslocal,sos1,sos2
        real(rkind), dimension(this%nxp,this%nyp,this%nzp) :: fluxu_int,fluxv_int, rhow_int, rhoe_int,spe_int, rhoYs_int, den, gradRYs, tauRho_mid, rhom, rhom_int,ke_int,sos_int,rhou_int,rhov_int,fluxw_int,H_int,delrhou,delrhov,H
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp) :: flux1,flux2,flux3,flux4,flux5,flux6,buff1,buff2,buff3,buff4,buff5,buff6
        ...
        p_int = this%p_mid(:,:,:,1)
        u_int = this%u_mid(:,:,:,1) !this%u_mid(:,:,:,1) !spec_int*fluxu_int !this%u_mid(:,:,:,1)
        v_int = this%v_mid(:,:,:,1)
        w_int = this%w_mid(:,:,:,1)
        ...
        do i = 1,2
           ...
           rhou_int = rhou_int + M_int(:,:,:,i)*u_int ! tmp1
           rhov_int = rhov_int + M_int(:,:,:,i)*v_int ! tmp2
           rhow_int = rhow_int + M_int(:,:,:,i)*w_int ! tmp3

        enddo



       fluxu_int = rhou_int*u_int
       fluxv_int = rhou_int*v_int
       fluxw_int = rhou_int*w_int
       KE  = 0.5_rkind*(u_int**2 + v_int**2 + w_int**2) ! ( rhou_int**2 + rhov_int**2 + rhow_int**2 ) / rho_int

      do i = 1,2

         e_prim = e_prim +  this%mix%material(i)%hydro%onebygam_m1*( p_int + this%mix%material(i)%hydro%gam*this%mix%material(i)%hydro%Pinf)/(M_int(:,:,:,i)/VF_int(:,:,:,i) ) *  M_int(:,:,:,i)/rho_int
      enddo

        H_int =(e_prim+p_int/rho_int+KE) 
        H = (this%e + this%p/this%rho + 0.5_rkind*(this%u**2 + this%v**2 + this%w**2))

        flux = 0.0
        buff = 0.25_rkind*fluxu_int + p_int - tauxx 
        buff1 =0.25_rkind*u_int*u_int
        buff2 = 0.5_rkind*rhou_int
        buff3 = 0.5_rkind*u_int
        buff4 = 0.25_rkind*rho_int
        ...
        call gradFV_x(this%decomp,this%derStagg,buff4,flux4,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        rhs(:,:,:,mom_index  ) = rhs(:,:,:,mom_index  ) - flux  -this%rho*flux1 - this%u*flux2 -this%rho*this%u*flux3 - this%u**2 * flux4 
        this%xflux_x = flux
        ...
        buff =0.25_rkind*fluxv_int - tauxy !y-momentum
        buff1=0.25_rkind*v_int*u_int
        buff2 =0.25_rkind*rhou_int
        buff3= 0.25_rkind*rhov_int
        buff4 = 0.25_rkind*u_int
        buff5 = 0.25_rkind*v_int
        buff6 = 0.25_rkind*rho_int
        !endif
        call gradFV_x(this%decomp,this%derStagg,buff,flux,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff1,flux1,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff2,flux2,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff3,flux3,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff4,flux4,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff5,flux5,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff6,flux6,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        rhs(:,:,:,mom_index+1) = rhs(:,:,:,mom_index+1) - flux -this%rho*flux1 -this%v*flux2 - this%u*flux3 &
                                 -this%rho*this%v*flux4 - this%rho*this%u*flux5-this%u*this%v*flux6

        this%xflux_y = flux

        buff = fluxw_int  - tauxz !z-momentum
        flux = 0.0
        ...
        !!!!!!!!!!!!!! add back in KE         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        buff = H_int*rhou_int -( tauxx)*u_int - (v_int*tauxy - w_int*tauxz)    
        buff1 = 0.25_rkind*rho_int*H_int
        buff2 = 0.25_rkind*rhou_int
        buff3 = 0.25_rkind*H_int*u_int
        buff4 = 0.25_rkind*H_int
        buff5 = 0.25_rkind*rho_int
        buff6 = 0.25_rkind*u_int
        call gradFV_x(this%decomp,this%derStagg,buff,flux,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff1,flux1,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff2,flux2,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff3,flux3,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        ...
        call gradFV_x(this%decomp,this%derStagg,buff5,flux5,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_x(this%decomp,this%derStagg,buff6,flux6,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc) 
        rhs(:,:,:, TE_index  ) = rhs(:,:,:, TE_index  ) - flux -this%u*flux1 - H*flux2  - this%rho*flux3 - this%rho*this%u*flux4 &
                                 - H*this%u*flux5 - H*this%rho*flux6
        this%xflux_e = flux
    ...
    subroutine getRHS_yStagg( this,  rhs, tauxy,tauyy,tauyz, qy)
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp,2) :: VF_int,M_int
        real(rkind), dimension(this%nxp,this%nyp,this%nzp) :: buff, flux,TE,u_int,v_int, w_int, p_int, tauxy_int, tauyy_int,tauyz_int, qy_int, e_int,rho_int, gam, num, KE, e_prim,gradu,gradup, UU, kef,delp,cl,cr,clocal,gradVF,gradVFx,gradVFy,UV,VV,tmp1,tmp2,tmp3,umag,soslocal,sos1,sos2
        real(rkind), dimension(this%nxp,this%nyp,this%nzp) :: fluxu_int,fluxv_int,rhow_int, rhoe_int, spe_int, rhoYs_int, den,ke_int, p4,Eint, up_int, cpressure, gradcpressure, gradp,sos_int, H_int, rhou_int,delrhou,delrhov,rhov_int,fluxw_int,H
       ...
       fluxu_int = rhov_int*u_int
       fluxv_int = rhov_int*v_int
       fluxw_int = rhov_int*w_int
       KE  = 0.5_rkind*(u_int**2 + v_int**2 + w_int**2 ) !  0.5_rkind* ( rhou_int**2 + rhov_int**2 + rhow_int**2 ) / rho_int 
...
!       rhoe_int = 0.5_rkind*KE*v_int +(  e_prim)*rhov_int

       H_int = ( KE + e_prim + p_int/rho_int)

       H = 0.5_rkind*(this%u**2 + this%v**2 + this%w**2 ) + this%p/this%rho + this%e

       flux  = 0.0
       buff  = 0.25_rkind*fluxu_int  - tauxy   !x-momentum 
       buff1 = 0.25_rkind*v_int*u_int
       buff2 = 0.25_rkind*rhou_int
       buff3 = 0.25_rkind*rhov_int
       buff4 = 0.25_rkind*u_int
       ...
       buff6 = 0.25_rkind*rho_int
        ...
        call gradFV_y(this%decomp,this%derStagg,buff4,flux4,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_y(this%decomp,this%derStagg,buff5,flux5,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_y(this%decomp,this%derStagg,buff6,flux6,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        rhs(:,:,:,mom_index )   = rhs(:,:,:,mom_index  ) - flux - this%rho*flux1 - this%u*flux2 - this%v*flux3 -this%rho*this%v*flux4 &
                                  -this%rho*this%u*flux5 - this%u*this%v*flux6

        this%yflux_x = flux

        flux = 0.0
        buff = 0.25_rkind*fluxv_int - tauyy + p_int  
        buff1 = 0.25_rkind*v_int*v_int
        buff2 = 0.5_rkind*rhov_int
        buff3 = 0.25_rkind*rho_int
        buff4 = 0.5_rkind*v_int
        call gradFV_y(this%decomp,this%derStagg,buff,flux,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_y(this%decomp,this%derStagg,buff1,flux1,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_y(this%decomp,this%derStagg,buff2,flux2,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_y(this%decomp,this%derStagg,buff3,flux3,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        call gradFV_y(this%decomp,this%derStagg,buff4,flux4,this%periodicx,this%periodicy,this%periodicz,this%x_bc,this%y_bc,this%z_bc)
        rhs(:,:,:,mom_index+1) = rhs(:,:,:,mom_index+1)  - flux  -this%rho*flux1 - this%v*flux2 - (this%v**2) *flux3 - this%rho*this%v*flux4
        this%yflux_y = flux

        flux = 0.0
        buff = 0.25*fluxw_int  - tauyz !z-momentum
        ...
        buff = H_int*rhov_int - (tauyy)*v_int - u_int*tauxy -w_int*tauyz 
        buff1 = 0.25_rkind*rho_int*H_int
        buff2 = 0.25_rkind*rhov_int
        buff3 = 0.25_rkind*H_int*v_int
        buff4 = 0.25_rkind*H_int
        buff5 = 0.25_rkind*rho_int
        buff6 = 0.25_rkind*v_int
        ...
        rhs(:,:,:, TE_index  ) = rhs(:,:,:, TE_index  ) - flux - this%v*flux1 - H*flux2  - this%rho*flux3 - this%rho*this%v*flux4 &
                                 - H*this%v*flux5 - H*this%rho*flux6
    ...
    subroutine getRHS_zStagg( this,  rhs, tauxz,tauyz,tauzz, qz)
        ...
        real(rkind), dimension(this%nxp,this%nyp,this%nzp) :: fluxu_int,fluxv_int, rhow_int, rhoe_int, spe_int, rhoYs_int, den, num,gam,e_prim
       ...
       do i = 1,2
          ...
          rho_int = rho_int + this%mix%material(i)%rhoYs_mid(:,:,:,3)
          fluxu_int = fluxu_int + this%mix%material(i)%rhoYs_mid(:,:,:,3)*u_int
          fluxv_int = fluxv_int + this%mix%material(i)%rhoYs_mid(:,:,:,3)*v_int
          rhow_int = rhow_int + this%mix%material(i)%rhoYs_mid(:,:,:,3)*w_int
          e_prim = e_prim + VF_int(:,:,:,i)*this%mix%material(i)%hydro%gam*this%mix%material(i)%hydro%onebygam_m1*(p_int + this%mix%material(i)%hydro%Pinf)
       ...
       enddo
        ...
        flux  = 0
        buff  = 0
        buff = rhow_int*u_int  - tauxz !x-momentum
        !buff = fluxu_int*w_int -tauxz_int
        ...
        flux = 0.0
        buff = rhow_int*v_int   - tauyz !y-momentum
        ...
        flux = 0.0
        buff = rhow_int*w_int + p_int   - tauzz !z-momentum
        ...
        KE =  half*(u_int*u_int + v_int*v_int + w_int*w_int)*rhow_int
        buff = ( e_prim + p_int  - tauzz )*w_int + KE - u_int*tauxz-v_int*tauyz ! + this%pu_mid(:,:,:,3)+w_int*rho_int*this%ke_mid(:,:,:,3) ! + qz
        !buff = ( TE + p_int )*w_int
