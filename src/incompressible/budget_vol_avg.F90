module budgets_vol_avg_mod
   ...
   use basic_io, only: read_2d_ascii, write_2d_ascii, write_1d_ascii, read_1d_ascii
   use mpi 
   ...
   ! BUDGET_1 term indices: (Reynolds Equations)
   ! For HIT :: all terms are identically zero
   ! For Homogeneous Shear Turbulence : some terms are non zero (should be implemented in the future)

   ! BUDGET_2 term indices: (MKE equation)
   ! For HIT :: all terms are identically zero
   ! For Homogeneous Shear Turbulence : some terms are non zero (should be implemented in the future)

   ! BUDGET_3 term indices: (TKE equation)
   ! For HIT ::
   !    1. SGS + Viscous dissipation 
   !    2. Forcing-velocity correlation
   ! For Homogeneous Shear Turbulence : (should be implemented in the future)
   !    1. SGS + Viscous dissipation 
   !    2. Forcing-velocity correlation
   !    3. TKE production
   ...
   type :: budgets_vol_avg
        private
        integer :: budgetType = 1, run_id, nz, num_vars_rz

        complex(rkind), dimension(:,:,:), allocatable :: uc, vc, wc, usgs, vsgs, wsgs, uvisc, vvisc, wvisc, px, py, pz, wb, ucor, vcor, wcor, uturb, HITforcing_x, HITforcing_y, HITforcing_z
        ...
        real(rkind), dimension(:), allocatable :: budget_3s, budget_3, budget_0
        integer :: counter
        real(rkind) :: avgFact 
    ...
    contains
        ...
        !procedure, private  :: get_vol_fluctE_from_fhatE
        !procedure, private  :: get_vol_fluctC_from_fhatC
        procedure, private  :: get_vol_meanC_from_fhatC 
        procedure, private  :: get_vol_meanE_from_fhatE 
        procedure, private  :: get_vol_meanC_from_fC 
        procedure, private  :: get_vol_meanE_from_fE 
...
contains 
    ...
    subroutine init(this, inputfile, igrid_sim) 
        ...
        namelist /BUDGET_VOL_AVG/ budgetType, budgets_dir, restart_budgets, restart_rid, restart_tid, restart_counter, tidx_dump, tidx_compute, do_budgets, tidx_budget_start, time_budget_start, do_spectra, do_autocorrel
        
        ! STEP 1: Read in inputs, link pointers and allocate budget vectors
        ioUnit = 534
        open(unit=ioUnit, file=trim(inputfile), form='FORMATTED', iostat=ierr)
        read(unit=ioUnit, NML=BUDGET_VOL_AVG)
        close(ioUnit)
        ...
        this%avgFact = 1.d0/(real(igrid_sim%nx,rkind)*real(igrid_sim%ny,rkind)*real(igrid_sim%nz,rkind))

        if((this%tidx_budget_start > 0) .and. (this%time_budget_start > 0.0d0)) then
            call GracefulExit("Both tidx_budget_start and time_budget_start in budget_vol_avg are positive. Turn one negative", 100)
        endif

        if(this%do_budgets) then
            !allocate(this%budget_0s(19))
            allocate(this%budget_0(19))
            !allocate(this%budget_1(this%nz,14))
            !allocate(this%budget_1s(this%nz,14))
            !allocate(this%budget_2(this%nz,7))
            allocate(this%budget_3(2))
            allocate(this%budget_3s(2))
            ...
            call igrid_sim%spectC%alloc_r2c_out(this%HITforcing_x)
            call igrid_sim%spectC%alloc_r2c_out(this%HITforcing_y)
            ...
            call igrid_sim%spectE%alloc_r2c_out(this%HITforcing_z)

            ! STEP 3: Now instrument igrid 
            call igrid_sim%instrumentForBudgets_volAvg(this%HITforcing_x, this%HITforcing_y, this%HITforcing_z)
    ...
    subroutine destroy(this)
        ...
        if(this%do_budgets) then    !---should we ideally check if (allocated(...)) for each array before deallocating ??
            ...
            deallocate(this%budget_0, this%budget_3, this%budget_3s)
            deallocate(this%HITforcing_x, this%HITforcing_y, this%HITforcing_z)
            if(this%do_spectra) then
    ...
    subroutine DumpBudget(this)
        ...
        if (nrank == 0) then
            ...
            call write_1d_ascii(this%budget_0, fname) 
            this%budget_0 = this%budget_0*(real(this%counter,rkind) + 1.d-18)
            ...
            if (this%budgetType>2) then
                ...
                fname = this%budgets_Dir(:len_trim(this%budgets_Dir))//"/"//trim(tempname)
                call write_1d_ascii(this%budget_3s, fname) 
            end if
...
!    end subroutine 
    ...
    subroutine restartBudget(this, rid, tid, cid)
        ...
        call read_1d_ascii(this%budget_0, fname)
        ...
        if (this%budgetType>2) then
            ...
            call read_1d_ascii(this%budget_3, fname)
    ...
    subroutine get_vol_meanC_from_fhatC(this, fhat, fmean)
        class(budgets_vol_avg), intent(inout) :: this
        ...
        real(rkind), intent(out) :: fmean
        integer :: ierr

        call transpose_y_to_z(fhat, this%igrid_sim%cbuffzC(:,:,:,1), this%igrid_sim%sp_gpC)
        call this%igrid_sim%spectC%take_fft1d_z2z_ip(this%igrid_sim%cbuffzC(:,:,:,1)) 

        if (nrank == 0) then
            fmean = real(this%igrid_sim%cbuffzC(1,1,1,1),rkind)*this%avgFact
        else
            fmean = 0.d0 ! Only 0 processor has the actual mean  
        end if
        
        call mpi_bcast(fmean,1,mpirkind,0,mpi_comm_world, ierr)

    end subroutine 

    subroutine get_vol_meanE_from_fhatE(this, fhat, fmean)
        class(budgets_vol_avg), intent(inout) :: this
        ...
        call this%igrid_sim%spectE%take_fft1d_z2z_ip(this%igrid_sim%cbuffzE(:,:,:,1)) 

        if (nrank == 0) then
            fmean = real(this%igrid_sim%cbuffzE(1,1,1,1),rkind)*this%avgFact
        else
            fmean = 0.d0 ! Only 0 processor has the actual mean  
        end if 
    ...
    subroutine get_vol_meanC_from_fC(this, f, fmean)
        class(budgets_vol_avg), intent(inout) :: this
    ...
    subroutine get_vol_meanE_from_fE(this, f, fmean)
        class(budgets_vol_avg), intent(inout) :: this
...
!    subroutine get_vol_fluctC_from_fhatC(this, fhat, ffluct)
!        class(budgets_vol_avg), intent(inout) :: this
!        complex(rkind), dimension(this%igrid_sim%sp_gpC%ysz(1), this%igrid_sim%sp_gpC%ysz(2), this%igrid_sim%sp_gpC%ysz(3)), intent(in) :: fhat
...
!    end subroutine 
!
!    subroutine get_vol_fluctE_from_fhatE(this, fhat, ffluct)
!        class(budgets_vol_avg), intent(inout) :: this
!        complex(rkind), dimension(this%igrid_sim%sp_gpE%ysz(1), this%igrid_sim%sp_gpE%ysz(2), this%igrid_sim%sp_gpE%ysz(3)), intent(in) :: fhat
...
!    end subroutine 
    ...
    subroutine AssembleBudget0(this)
        ...
        ! STEP 1: Get Reynolds stresses (IMPORTANT: need to correct for fluctuation before dumping)
        this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%u*this%igrid_sim%u
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(1) = this%budget_0(1) + tmpvar

        this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%u*this%igrid_sim%v
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(2) = this%budget_0(2) + tmpvar
        
        this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%u*this%igrid_sim%wC
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(3) = this%budget_0(3) + tmpvar
        
        this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%v*this%igrid_sim%v
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(4) = this%budget_0(4) + tmpvar

        this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%v*this%igrid_sim%wC
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(5) = this%budget_0(5) + tmpvar
        
        this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%wC*this%igrid_sim%wC
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(6) = this%budget_0(6) + tmpvar
        ...
        if(this%igrid_sim%isStratified) then
            ...
            call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
            this%budget_0(7) = this%budget_0(7) + tmpvar
            
            this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%v*this%igrid_sim%T
            call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
            this%budget_0(8) = this%budget_0(8) + tmpvar

            this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%T*this%igrid_sim%wC
            call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
            this%budget_0(9) = this%budget_0(9) + tmpvar

            this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%T*this%igrid_sim%T
            call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
            this%budget_0(10) = this%budget_0(10) + tmpvar
        endif

        ! STEP 3: SGS stress (also viscous stress if finite reynolds number is being used)
        call this%igrid_sim%Pade6opZ%interpz_E2C(this%igrid_sim%tau13, this%igrid_sim%rbuffxC(:,:,:,1), 0, 0)
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(11) = this%budget_0(11) + tmpvar

        call this%igrid_sim%Pade6opZ%interpz_E2C(this%igrid_sim%tau23, this%igrid_sim%rbuffxC(:,:,:,1), 0, 0)
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_0(12) = this%budget_0(12) + tmpvar

        if (associated(this%igrid_sim%q1)) then
            call this%get_vol_meanC_from_fC(this%igrid_sim%q1, tmpvar)
            this%budget_0(13) = this%budget_0(13) + tmpvar
        end if 

        if (associated(this%igrid_sim%q2)) then
            call this%get_vol_meanC_from_fC(this%igrid_sim%q2, tmpvar)
            this%budget_0(14) = this%budget_0(14) + tmpvar
        end if 
        ...
        if (associated(this%igrid_sim%q3)) then
            ...
            call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
            this%budget_0(15) = this%budget_0(15) + tmpvar
        end if 

        ! Get mean tau_11
        call this%get_vol_meanC_from_fC(this%igrid_sim%tauSGS_ij(:,:,:,1), tmpvar)
        this%budget_0(16) = this%budget_0(16) + tmpvar

        ! Get mean tau_12
        call this%get_vol_meanC_from_fC(this%igrid_sim%tauSGS_ij(:,:,:,2), tmpvar)
        this%budget_0(17) = this%budget_0(17) + tmpvar

        ! Get mean tau_22
        call this%get_vol_meanC_from_fC(this%igrid_sim%tauSGS_ij(:,:,:,4), tmpvar)
        this%budget_0(18) = this%budget_0(18) + tmpvar

        ! Get mean tau_33
        call this%get_vol_meanC_from_fC(this%igrid_sim%tauSGS_ij(:,:,:,6), tmpvar)
        this%budget_0(19) = this%budget_0(19) + tmpvar
    ...
    subroutine AssembleBudget1(this)
        ...
        !call this%get_vol_meanC_from_fhatC(this%uc, this%tmp_meanC)
        !this%budget_1(:,1) = this%budget_1(:,1) + this%tmp_meanC

        !call this%get_vol_meanC_from_fhatC(this%usgs, this%tmp_meanC)
        !this%budget_1(:,2) = this%budget_1(:,2) + this%tmp_meanC
        !
        !call this%get_vol_meanC_from_fhatC(this%uvisc, this%tmp_meanC)
        !this%budget_1(:,3) = this%budget_1(:,3) + this%tmp_meanC

        !call this%get_vol_meanC_from_fhatC(this%ucor, this%tmp_meanC)
        ...
        !call this%get_vol_meanC_from_fhatC(this%uturb, this%tmp_meanC)
        !this%budget_1(:,6) = this%budget_1(:,6) + this%tmp_meanC
       
        !! Get y- momentum budget 
        !call this%get_vol_meanC_from_fhatC(this%vc, this%tmp_meanC)
        !this%budget_1(:,7) = this%budget_1(:,7) + this%tmp_meanC

        !call this%get_vol_meanC_from_fhatC(this%vsgs, this%tmp_meanC)
        !this%budget_1(:,8) = this%budget_1(:,8) + this%tmp_meanC

        !call this%get_vol_meanC_from_fhatC(this%vvisc, this%tmp_meanC)
        !this%budget_1(:,9) = this%budget_1(:,9) + this%tmp_meanC

        !call this%get_vol_meanC_from_fhatC(this%vcor, this%tmp_meanC)
        ...
        !call this%get_vol_meanE_from_fhatE(this%wc, this%tmp_meanE)
        !call this%interp_1d_Edge2Cell(this%tmp_meanE, this%tmp_meanC,0,0)
        !this%budget_1(:,12) = this%budget_1(:,12) + this%tmp_meanC

        !call this%get_vol_meanE_from_fhatE(this%wsgs, this%tmp_meanE)
        !call this%interp_1d_Edge2Cell(this%tmp_meanE, this%tmp_meanC,0,0)
        !this%budget_1(:,13) = this%budget_1(:,13) + this%tmp_meanC

        !call this%get_vol_meanE_from_fhatE(this%pz, this%tmp_meanE)
        !call this%interp_1d_Edge2Cell(this%tmp_meanE, this%tmp_meanC,0,0)
    ...
    subroutine AssembleBudget3(this)
        ...
        !call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,4),this%tmp_meanC)
        ...
        !call this%get_vol_meanE_from_fE(this%igrid_sim%rbuffxE(:,:,:,1), this%tmp_meanE)
        ...
        call this%igrid_sim%Pade6opZ%interpz_E2C(this%igrid_sim%rbuffxE(:,:,:,1), this%igrid_sim%rbuffxC(:,:,:,2), 0, 0)
        this%igrid_sim%rbuffxC(:,:,:,1) = this%igrid_sim%rbuffxC(:,:,:,1) + this%igrid_sim%rbuffxC(:,:,:,2)

        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_3(1) = this%budget_3(1) + tmpvar

        ! HIT forcing
        !print '(a,i4.4,3(1x,e19.12))', '++++++++', nrank, maxval(abs(this%HITforcing_x)), maxval(abs(this%HITforcing_y)), maxval(abs(this%HITforcing_z))
        call this%igrid_sim%spectC%ifft(this%HITforcing_x,this%igrid_sim%rbuffxC(:,:,:,1))
        this%igrid_sim%rbuffxC(:,:,:,2) = this%igrid_sim%rbuffxC(:,:,:,1)*this%igrid_sim%u
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,2), tmpvar)
        this%budget_3(2) = this%budget_3(2)  + tmpvar

        call this%igrid_sim%spectC%ifft(this%HITforcing_y,this%igrid_sim%rbuffxC(:,:,:,1))
        this%igrid_sim%rbuffxC(:,:,:,2) = this%igrid_sim%rbuffxC(:,:,:,1)*this%igrid_sim%v
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,2), tmpvar)
        this%budget_3(2) = this%budget_3(2)  + tmpvar

        call this%igrid_sim%spectE%ifft(this%HITforcing_z,this%igrid_sim%rbuffxE(:,:,:,1))
        this%igrid_sim%rbuffxE(:,:,:,2) = this%igrid_sim%rbuffxE(:,:,:,1)*this%igrid_sim%w
        call this%igrid_sim%Pade6opZ%interpz_E2C(this%igrid_sim%rbuffxE(:,:,:,2), this%igrid_sim%rbuffxC(:,:,:,1), 0, 0)
        call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1), tmpvar)
        this%budget_3(2) = this%budget_3(2)  + tmpvar
    ...
    subroutine AssembleBudget4_13(this)
        ...
        !call this%get_vol_meanC_from_fC(this%igrid_sim%rbuffxC(:,:,:,1),this%tmp_meanC)
