subroutine init_SGSnn(this,NNtype)
   ...
   select case (NNtype)
   case (1) ! Unet
     allocate(this%invariants(1,this%gpC%xsz(1),this%gpC%xsz(3),this%gpC%xsz(2),3))
     allocate(this%invariantsnorm(1,this%gpC%xsz(1),this%gpC%xsz(3),this%gpC%xsz(2),3))
     ...
     allocate(this%invariantsE(1,this%gpE%xsz(1),this%gpE%xsz(3),this%gpE%xsz(2),3))
     allocate(this%invariantsEnorm(1,this%gpE%xsz(1),this%gpE%xsz(3),this%gpE%xsz(2),3))
...
subroutine getSijRijforNNmod(duidxj, Sij, Rij, nxL, nyL, nzL)
   ...
   do k = 1,nzL
      do j = 1,nyL
         !$omp simd
         do i = 1,nxL
            ! print *, duidxj(i,j,k,1)+duidxj(i,j,k,5)+duidxj(i,j,k,9) 
...
subroutine compute_tauij_NN(this,tidNow)
  ...
  del = ((this%dx*this%dy*this%dz)**(1.0/3.0)*f)**2.0
    ...
    do j = 1, this%gpC%xsz(2)
      do i = 1, this%gpC%xsz(1)
        ...
        if (this%NNtype == 1) then
          ...
          this%invariants(1, i, k, j, 2) = norm2(S_ij_a(:))
          this%invariants(1, i, k, j, 3) = norm2(R_ij_a(:))
     ...
     do j = 1, this%gpE%xsz(2)
       do i = 1, this%gpE%xsz(1)
         ...
         if (this%NNtype == 1) then
           ...
           this%invariantsE(1, i, k, j, 2) = norm2(S_ij_a(:))
           this%invariantsE(1, i, k, j, 3) = norm2(R_ij_a(:))
  ...
  trace = finalout(1,1,:,:,:)*del*magout(1,:,:,:)**2+finalout(1,4,:,:,:)*del*magout(1,:,:,:)**2+finalout(1,6,:,:,:)*del*magout(1,:,:,:)**2
  this%tau_11 = finalout(1,1,:,:,:)*del*magout(1,:,:,:)**2-1.0/3.0*(trace)
  this%tau_12 = finalout(1,2,:,:,:)*del*magout(1,:,:,:)**2
  this%tau_13 = finaloutE(1,3,:,:,:)*del*magoutE(1,:,:,:)**2
  this%tau_22 = finalout(1,4,:,:,:)*del*magout(1,:,:,:)**2-1.0/3.0*(trace)
  this%tau_23 = finaloutE(1,5,:,:,:)*del*magoutE(1,:,:,:)**2
  this%tau_33 = finalout(1,6,:,:,:)*del*magout(1,:,:,:)**2-1.0/3.0*(trace)
