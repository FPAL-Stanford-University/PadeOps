module CurvilDirLADCompressibleGrid
    ...
    type, extends(grid) :: cvldirladgrid
        ...
        real(rkind), dimension(:,:,:), pointer     :: delta_Lxi, delta_Leta, delta_Lzeta
...
contains
    ...
    subroutine init_curvilinear(this, inputfile)
        ...
        this%InvJ = 1 / (this%InvJ + real(1.0D-32,rkind) )
            ...
            pmax = P_MAXVAL(abs(dxidxij(:,:,:,2)))
            if (nrank == 0) then
                print*, '>>> Max value of I1 <<<', pmax
            end if
        ...
        do while ( tcond .AND. stepcond )
            ...
            if(this%useSGS) then
              call hook_timestep(this%decomp, this%mesh, this%fields, this%mix, this%step, this%tsim, &
                                this%LAD_mustar, this%LAD_bulkstar, this%LAD_kappastar, this%sgsmodel)
            else
              call hook_timestep(this%decomp, this%mesh, this%fields, this%mix, this%step, this%tsim , &
                                this%LAD_mustar, this%LAD_bulkstar, this%LAD_kappastar)
            endif
    ...
    subroutine get_dt(this,stability)
        ...
        real(rkind) :: dtmu_xi, dtmu_eta, dtmu_zeta, dtbulk_xi, dtbulk_eta, dtbulk_zeta, dtkap_xi, dtkap_eta, dtkap_zeta
    ...
    subroutine getLAD(this, dudx, dudy, dudz,&
        ...
        call this%der%d2dxi2(xtmp2,xtmp1,this%x_bc(1),this%x_bc(2))
        call transpose_x_to_y(xtmp1,ytmp1,this%decomp) 
        mustar = ytmp1*(this%dxi**4)*(this%dxs**2)
        ...
        call this%der%d2dzeta2(ztmp2,ztmp1,this%z_bc(1),this%z_bc(2))
        call transpose_z_to_y(ztmp1,ytmp1,this%decomp)
        ytmp2 = ytmp1*(this%dzeta**4)*(this%dzs**2)
        mustar = mustar + ytmp2
        
        ! Get 4th derivative in Y
        call this%der%d2deta2(func,ytmp1,this%y_bc(1),this%y_bc(2))
        call this%der%d2deta2(ytmp1,ytmp2,this%y_bc(1),this%y_bc(2))
        ytmp1 = ytmp2*(this%deta**4)*(this%dys**2)
        mustar = mustar + ytmp1
        ...
        call this%gradient_cvl(this%rho,ytmp1,ytmp2,ytmp3,this%x_bc,this%y_bc,this%z_bc) ! Does not use any Y buffers
        ...
        bulkstar = bulkstar + ytmp4*ytmp3

        ! Step 4: Get 4th derivative in Y
        call this%der%d2deta2(func,ytmp4,this%y_bc(1),this%y_bc(2))
        call this%der%d2deta2(ytmp4,ytmp5,this%y_bc(1),this%y_bc(2))
        ytmp4 = ytmp5*(this%deta**4)
        bulkstar = bulkstar + ytmp4*ytmp2
        bulkstar = abs(bulkstar) * this%Cbeta*this%rho
        ! Filter bulkstar
        call this%filter(bulkstar, this%gfil, 2, this%x_bc, this%y_bc, this%z_bc)
        ytmp1 =  bulkstar * this%delta_Lxi
        ytmp2 =  bulkstar * this%delta_Leta
        ytmp3 =  bulkstar * this%delta_Lzeta
  
        this%LAD_bulkstar(:,:,:,1) = ytmp1
        ...
        xtmp2 = xtmp1*this%dxi**4
        call transpose_x_to_y(xtmp2,ytmp4,this%decomp)
        kapstar = ytmp4 * ( this%dxs * sqrt(ytmp1 / (ytmp1 + ytmp2 + ytmp3 + real(1.0D-32,rkind))) ) 
        ...
        ztmp2 = ztmp1*this%dzeta**4
        call transpose_z_to_y(ztmp2,ytmp4,this%decomp)
        kapstar = kapstar + ytmp4 * ( this%dzs * sqrt(ytmp3 / (ytmp1 + ytmp2 + ytmp3 + real(1.0D-32,rkind))) )

        ! Step 4: Get 4th derivative in Y
        call this%der%d2deta2(this%e,ytmp4,this%y_bc(1),this%y_bc(2))
        call this%der%d2deta2(ytmp4,ytmp5,this%y_bc(1),this%y_bc(2))
        ytmp4 = ytmp5*this%deta**4
        kapstar = kapstar + ytmp4 * (this%dys * sqrt(ytmp2/(ytmp1 + ytmp2 + ytmp3 + real(1.0D-32,rkind))) ) 
        ...
        if (present(dYsdx) .AND. present(dYsdy) .AND. present(dYsdz)) then
            if (this%mix%ns .GT. 1) then
                do p = 1,this%mix%ns
                    ...
                    xtmp2 = xtmp1*this%dxi**4
                    call transpose_x_to_y(xtmp2,ytmp4,this%decomp)
                    diffstar = ytmp4 * ( this%dxs * sqrt(ytmp1 / (ytmp1 + ytmp2 + ytmp3 + real(1.0D-32,rkind))) ) 
                    ...
                    ztmp2 = ztmp1*this%dzeta**4
                    call transpose_z_to_y(ztmp2,ytmp4,this%decomp)
                    diffstar = diffstar + ytmp4 * ( this%dzs * sqrt(ytmp3 / (ytmp1 + ytmp2 + ytmp3 + real(1.0D-32,rkind))) )

                    ! Step 4: Get 4th derivative in Y
                    call this%der%d2deta2(this%Ys(:,:,:,p),ytmp4,this%y_bc(1),this%y_bc(2))
                    call this%der%d2deta2(ytmp4,ytmp5,this%y_bc(1),this%y_bc(2))
                    ytmp4 = ytmp5*this%deta**4
                    diffstar = diffstar + ytmp4 * ( this%dys * sqrt(ytmp2 / (ytmp1 + ytmp2 + ytmp3 + real(1.0D-32,rkind))) ) 

                    diffstar = this%Cdiff*ytmp6*abs(diffstar)     ! CD part of diff
                    ytmp4 = (sqrt(ytmp1)*this%dxs + sqrt(ytmp2)*this%dys + sqrt(ytmp3)*this%dzs) &
