...
subroutine get_modS_Sii(this, modS_sq, Sii)
   ...
   modS_sq = modS_sq + two * this%S_ij(:,:,:,2) * this%S_ij(:,:,:,2)  ! S12*S12 + S21*S21
   modS_sq = modS_sq + two * this%S_ij(:,:,:,3) * this%S_ij(:,:,:,3)  ! S13*S13 + S31*S31
   modS_sq = modS_sq +       this%S_ij(:,:,:,4) * this%S_ij(:,:,:,4)  ! S22*S22
   modS_sq = modS_sq + two * this%S_ij(:,:,:,5) * this%S_ij(:,:,:,5)  ! S23*S23 + S32*S32
   modS_sq = modS_sq +       this%S_ij(:,:,:,6) * this%S_ij(:,:,:,6)  ! S33*S33
...
subroutine get_SGS_kernel(this,duidxj, modS_sq)
   ...
   case (0)
      ! Smagorinsky
      call this%get_smagorinsky_kernel(modS_sq)
   case (1)
      ! Sigma
      call this%get_sigma_kernel(duidxj)
   case (2)
      ! AMD  
