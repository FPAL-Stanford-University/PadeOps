...
module cfo2stuff
    ...
    private
    public :: cfo2, alphao2, betao2, ao2, bo2, co2, do2, eo2
    
    !cfo2 -- second order filter with adjustable alpha

    !cfo2
    ! real(rkind), parameter :: alphao2= 0.475 ! Already divided by factor of 2 
    ! real(rkind), parameter :: betao2 = 0.0
    ! real(rkind), parameter :: ao2    = 0.975
    ! real(rkind), parameter :: bo2    = 0.4875 ! Already divided by factor of 2 
    ! real(rkind), parameter :: co2    = 0.0
    ! real(rkind), parameter :: do2    = 0.0
    ! real(rkind), parameter :: eo2    = 0.0


    real(rkind) :: alphao2= 0.475 ! Already divided by factor of 2 
    real(rkind) :: betao2 = 0.0
    real(rkind) :: ao2    = 0.975
    real(rkind) :: bo2    = 0.4875 ! Already divided by factor of 2 
    real(rkind) :: co2    = 0.0
    real(rkind) :: do2    = 0.0
    real(rkind) :: eo2    = 0.0

    !Boundary filters same as interior

    ! !as functions of alphao2
    ! ao2 = (two*alphao2+one)/two
    ! bo2 = (two*alphao2+one)/four
...
contains
    ...
    function init(this, n_, periodic_, alphao2_) result(ierr)
   
        class( cfo2 ), intent(inout) :: this
        integer, intent(in) :: n_
        logical, intent(in) :: periodic_
        real(rkind), intent(in) :: alphao2_
        integer :: ierr
        ...
        if (periodic_) then 
            ...
            ! Compute LU matrix
            if (n_ .GE. 10) then
                call this%ComputeLU(this%LU,betao2,alphao2,one,alphao2,betao2)
            else if (n_ == 1) then
                this%LU = one
            else
                ierr = 7
    ...
    subroutine ComputePenta(this,penta,bc1,bcn)
        ...
        associate (bt   => penta(:,1), b   => penta(:,2), d => penta(:,3),  &
            ...
            case(0)
                ...
                at(this%n-3) = betao2
            case(1)
                bt(this%n  ) = two*betao2
                b (this%n  ) = two*alphao2
                d (this%n  ) = one
                a (this%n  ) = zero 
                at(this%n  ) = zero
...
#include "CFo2_files/SolveZPenta_common.F90"        
...
#include "CFo2_files/SolveZPenta_common.F90"        
    ...
    pure subroutine ComputeXRHS(this, f, RHS, n2, n3, bc1, bcn)
        ...
        case (.TRUE.)
            do k=1,n3
                do j=1,n2
                    RHS(         1,j,k) = ao2 * ( f(         1,j,k) )                     &
                                        + bo2 * ( f(         2,j,k) + f(    this%n,j,k) ) &
                                        + co2 * ( f(         3,j,k) + f(  this%n-1,j,k) ) &
                                        + do2 * ( f(         4,j,k) + f(  this%n-2,j,k) ) &
                                        + eo2 * ( f(         5,j,k) + f(  this%n-3,j,k) )
                    RHS(         2,j,k) = ao2 * ( f(         2,j,k) )                     &
                                        + bo2 * ( f(         3,j,k) + f(         1,j,k) ) &
                                        + co2 * ( f(         4,j,k) + f(    this%n,j,k) ) &
                                        + do2 * ( f(         5,j,k) + f(  this%n-1,j,k) ) &
                                        + eo2 * ( f(         6,j,k) + f(  this%n-2,j,k) )
                    RHS(         3,j,k) = ao2 * ( f(         3,j,k) )                     &
                                        + bo2 * ( f(         4,j,k) + f(         2,j,k) ) &
                                        + co2 * ( f(         5,j,k) + f(         1,j,k) ) &
                                        + do2 * ( f(         6,j,k) + f(    this%n,j,k) ) &
                                        + eo2 * ( f(         7,j,k) + f(  this%n-1,j,k) )
                    RHS(         4,j,k) = ao2 * ( f(         4,j,k) )                     &
                                        + bo2 * ( f(         5,j,k) + f(         3,j,k) ) &
                                        + co2 * ( f(         6,j,k) + f(         2,j,k) ) &
                                        + do2 * ( f(         7,j,k) + f(         1,j,k) ) &
                                        + eo2 * ( f(         8,j,k) + f(    this%n,j,k) )
                    RHS(5:this%n-4,j,k) = ao2 * ( f(5:this%n-4,j,k) )                     &
                                        + bo2 * ( f(6:this%n-3,j,k) + f(4:this%n-5,j,k) ) &
                                        + co2 * ( f(7:this%n-2,j,k) + f(3:this%n-6,j,k) ) &
                                        + do2 * ( f(8:this%n-1,j,k) + f(2:this%n-7,j,k) ) &
                                        + eo2 * ( f(9:this%n  ,j,k) + f(1:this%n-8,j,k) )
                    RHS(  this%n-3,j,k) = ao2 * ( f(  this%n-3,j,k) )                     &
                                        + bo2 * ( f(  this%n-2,j,k) + f(  this%n-4,j,k) ) &
                                        + co2 * ( f(  this%n-1,j,k) + f(  this%n-5,j,k) ) &
                                        + do2 * ( f(    this%n,j,k) + f(  this%n-6,j,k) ) &
                                        + eo2 * ( f(         1,j,k) + f(  this%n-7,j,k) )
                    RHS(  this%n-2,j,k) = ao2 * ( f(  this%n-2,j,k) )                     &
                                        + bo2 * ( f(  this%n-1,j,k) + f(  this%n-3,j,k) ) &
                                        + co2 * ( f(    this%n,j,k) + f(  this%n-4,j,k) ) &
                                        + do2 * ( f(         1,j,k) + f(  this%n-5,j,k) ) &
                                        + eo2 * ( f(         2,j,k) + f(  this%n-6,j,k) )
                    RHS(  this%n-1,j,k) = ao2 * ( f(  this%n-1,j,k) )                     &
                                        + bo2 * ( f(    this%n,j,k) + f(  this%n-2,j,k) ) &
                                        + co2 * ( f(         1,j,k) + f(  this%n-3,j,k) ) &
                                        + do2 * ( f(         2,j,k) + f(  this%n-4,j,k) ) &
                                        + eo2 * ( f(         3,j,k) + f(  this%n-5,j,k) )
                    RHS(    this%n,j,k) = ao2 * ( f(    this%n,j,k) )                     &
                                        + bo2 * ( f(         1,j,k) + f(  this%n-1,j,k) ) &
                                        + co2 * ( f(         2,j,k) + f(  this%n-2,j,k) ) &
                                        + do2 * ( f(         3,j,k) + f(  this%n-3,j,k) ) &
                                        + eo2 * ( f(         4,j,k) + f(  this%n-4,j,k) )
                end do
            end do
                ...
                do j = 1,n2
                    ...
                    case(1)
                        RHS(this%n-3,j,k) =    ao2 * ( f(this%n-3,j,k) )                   &
                                          ...
                                          +    co2 * ( f(this%n-1,j,k) + f(this%n-5,j,k) ) &
                                          +    do2 * ( f(this%n  ,j,k) + f(this%n-6,j,k) ) &
                                          +    eo2 * ( f(this%n-1,j,k) + f(this%n-7,j,k) )
                        RHS(this%n-2,j,k) =    ao2 * ( f(this%n-2,j,k) )                   &
                                          +    bo2 * ( f(this%n-1,j,k) + f(this%n-3,j,k) ) &
                                          +    co2 * ( f(this%n  ,j,k) + f(this%n-4,j,k) ) &
                                          +    do2 * ( f(this%n-1,j,k) + f(this%n-5,j,k) ) &
                                          +    eo2 * ( f(this%n-2,j,k) + f(this%n-6,j,k) )
                        RHS(this%n-1,j,k) =    ao2 * ( f(this%n-1,j,k) )                   &
                                          +    bo2 * ( f(this%n  ,j,k) + f(this%n-2,j,k) ) &
                                          +    co2 * ( f(this%n-1,j,k) + f(this%n-3,j,k) ) &
                                          +    do2 * ( f(this%n-2,j,k) + f(this%n-4,j,k) ) &
                                          +    eo2 * ( f(this%n-3,j,k) + f(this%n-5,j,k) )
                        RHS(this%n  ,j,k) =    ao2 * ( f(this%n  ,j,k) )                   &
                                          +    bo2 * ( f(this%n-1,j,k) + f(this%n-1,j,k) ) &
                                          +    co2 * ( f(this%n-2,j,k) + f(this%n-2,j,k) ) &
                                          +    do2 * ( f(this%n-3,j,k) + f(this%n-3,j,k) ) &
                                          +    eo2 * ( f(this%n-4,j,k) + f(this%n-4,j,k) )
                    case(-1)
                        RHS(this%n-3,j,k) =    ao2 * ( f(this%n-3,j,k) )                   &
                                          +    bo2 * ( f(this%n-2,j,k) + f(this%n-4,j,k) ) &
                                          +    co2 * ( f(this%n-1,j,k) + f(this%n-5,j,k) ) &
    ...
    pure subroutine ComputeYRHS(this, f, RHS, n1, n3, bc1, bcn) 
        ...
        case (.TRUE.)
            do k=1,n3
                RHS(:,         1,k) = ao2 * ( f(:,         1,k) )                     &
                                    + bo2 * ( f(:,         2,k) + f(:,    this%n,k) ) &
                                    + co2 * ( f(:,         3,k) + f(:,  this%n-1,k) ) &
                                    + do2 * ( f(:,         4,k) + f(:,  this%n-2,k) ) &
                                    + eo2 * ( f(:,         5,k) + f(:,  this%n-3,k) )
                RHS(:,         2,k) = ao2 * ( f(:,         2,k) )                     &
                                    + bo2 * ( f(:,         3,k) + f(:,         1,k) ) &
                                    + co2 * ( f(:,         4,k) + f(:,    this%n,k) ) &
                                    + do2 * ( f(:,         5,k) + f(:,  this%n-1,k) ) &
                                    + eo2 * ( f(:,         6,k) + f(:,  this%n-2,k) )
                RHS(:,         3,k) = ao2 * ( f(:,         3,k) )                     &
                                    + bo2 * ( f(:,         4,k) + f(:,         2,k) ) &
                                    + co2 * ( f(:,         5,k) + f(:,         1,k) ) &
                                    + do2 * ( f(:,         6,k) + f(:,    this%n,k) ) &
                                    + eo2 * ( f(:,         7,k) + f(:,  this%n-1,k) )
                RHS(:,         4,k) = ao2 * ( f(:,         4,k) )                     &
                                    + bo2 * ( f(:,         5,k) + f(:,         3,k) ) &
                                    + co2 * ( f(:,         6,k) + f(:,         2,k) ) &
                                    + do2 * ( f(:,         7,k) + f(:,         1,k) ) &
                                    + eo2 * ( f(:,         8,k) + f(:,    this%n,k) )
                RHS(:,5:this%n-4,k) = ao2 * ( f(:,5:this%n-4,k) )                     &
                                    + bo2 * ( f(:,6:this%n-3,k) + f(:,4:this%n-5,k) ) &
                                    + co2 * ( f(:,7:this%n-2,k) + f(:,3:this%n-6,k) ) &
                                    + do2 * ( f(:,8:this%n-1,k) + f(:,2:this%n-7,k) ) &
                                    + eo2 * ( f(:,9:this%n  ,k) + f(:,1:this%n-8,k) )
                RHS(:,  this%n-3,k) = ao2 * ( f(:,  this%n-3,k) )                     &
                                    + bo2 * ( f(:,  this%n-2,k) + f(:,  this%n-4,k) ) &
                                    + co2 * ( f(:,  this%n-1,k) + f(:,  this%n-5,k) ) &
                                    + do2 * ( f(:,    this%n,k) + f(:,  this%n-6,k) ) &
                                    + eo2 * ( f(:,         1,k) + f(:,  this%n-7,k) )
                RHS(:,  this%n-2,k) = ao2 * ( f(:,  this%n-2,k) )                     &
                                    + bo2 * ( f(:,  this%n-1,k) + f(:,  this%n-3,k) ) &
                                    + co2 * ( f(:,    this%n,k) + f(:,  this%n-4,k) ) &
                                    + do2 * ( f(:,         1,k) + f(:,  this%n-5,k) ) &
                                    + eo2 * ( f(:,         2,k) + f(:,  this%n-6,k) )
                RHS(:,  this%n-1,k) = ao2 * ( f(:,  this%n-1,k) )                     &
                                    + bo2 * ( f(:,    this%n,k) + f(:,  this%n-2,k) ) &
                                    + co2 * ( f(:,         1,k) + f(:,  this%n-3,k) ) &
                                    + do2 * ( f(:,         2,k) + f(:,  this%n-4,k) ) &
                                    + eo2 * ( f(:,         3,k) + f(:,  this%n-5,k) )
                RHS(:,    this%n,k) = ao2 * ( f(:,    this%n,k) )                     &
                                    + bo2 * ( f(:,         1,k) + f(:,  this%n-1,k) ) &
                                    + co2 * ( f(:,         2,k) + f(:,  this%n-2,k) ) &
                                    + do2 * ( f(:,         3,k) + f(:,  this%n-3,k) ) &
                                    + eo2 * ( f(:,         4,k) + f(:,  this%n-4,k) )
            end do
        case (.FALSE.)
            ...
            do k = 1,n3
                ...
                case(1)
                    RHS(:,this%n-3,k) =    ao2 * ( f(:,this%n-3,k) )                   &
                                      ...
                                      +    co2 * ( f(:,this%n-1,k) + f(:,this%n-5,k) ) &
                                      +    do2 * ( f(:,this%n  ,k) + f(:,this%n-6,k) ) &
                                      +    eo2 * ( f(:,this%n-1,k) + f(:,this%n-7,k) )
                    RHS(:,this%n-2,k) =    ao2 * ( f(:,this%n-2,k) )                   &
                                      +    bo2 * ( f(:,this%n-1,k) + f(:,this%n-3,k) ) &
                                      +    co2 * ( f(:,this%n  ,k) + f(:,this%n-4,k) ) &
                                      +    do2 * ( f(:,this%n-1,k) + f(:,this%n-5,k) ) &
                                      +    eo2 * ( f(:,this%n-2,k) + f(:,this%n-6,k) )
                    RHS(:,this%n-1,k) =    ao2 * ( f(:,this%n-1,k) )                   &
                                      +    bo2 * ( f(:,this%n  ,k) + f(:,this%n-2,k) ) &
                                      +    co2 * ( f(:,this%n-1,k) + f(:,this%n-3,k) ) &
                                      +    do2 * ( f(:,this%n-2,k) + f(:,this%n-4,k) ) &
                                      +    eo2 * ( f(:,this%n-3,k) + f(:,this%n-5,k) )
                    RHS(:,this%n  ,k) =    ao2 * ( f(:,this%n  ,k) )                   &
                                      +    bo2 * ( f(:,this%n-1,k) + f(:,this%n-1,k) ) &
                                      +    co2 * ( f(:,this%n-2,k) + f(:,this%n-2,k) ) &
                                      +    do2 * ( f(:,this%n-3,k) + f(:,this%n-3,k) ) &
                                      +    eo2 * ( f(:,this%n-4,k) + f(:,this%n-4,k) )
                case(-1)  
                    RHS(:,this%n-3,k) =    ao2 * ( f(:,this%n-3,k) )                   &
                                      +    bo2 * ( f(:,this%n-2,k) + f(:,this%n-4,k) ) &
                                      +    co2 * ( f(:,this%n-1,k) + f(:,this%n-5,k) ) &
...
#include "CFo2_files/ComputeZRHS_common.F90"    
