...
module cf80stuff
    ...
    private
    public :: cf80, alpha80, beta80, a80, b80, c80, d80, e80
    
    !cf80 has smame transfer function value (0.579032) at 0.8pi (2.5ppw) as cf90 has at 0.9pi (2.2ppw). cf80 boundary filters have same transfter function value at 0.8pi as cf80 interior filter
    ! ! 8th order filter coefficients with 80% truncation (See Lele (1992) for explanation)
    ...
    !cf80
    real(rkind), parameter :: alpha80= 0.6581136121367406!0.49875!0.9975!0.9985!0.9995!real(6.7D-1, rkind)
    real(rkind), parameter :: beta80 = 0.17094319393162943!0.0!real(1.8D-1, rkind)
    real(rkind), parameter :: a80    = 0.99298382245592!1.0!0.9993077734375002!real(0.99965, rkind)
    ...
    ! Second point
    real(rkind), parameter :: b2_alpha80= 0.4287176849414513!0.49875!.9975!0.9985!0.9995!real( 4.997D-1, rkind)
    real(rkind), parameter :: b2_a80    = 0.9287176849414513!1.0!real( 9.997D-1, rkind)
    real(rkind), parameter :: b2_b80    = 0.4643588424707257!0.49875!0.9975!0.9985!0.9995!real(4.9985D-1, rkind) ! Already divided by factor of 2 
    
    ! Third point
    real(rkind), parameter :: b3_alpha80= 0.6559494020913718!0.49875!0.9975!0.9985!0.9995!real(6.6624D-1, rkind)
    real(rkind), parameter :: b3_beta80 = 0.16666666666666666!0.0!real(1.6688D-1, rkind)
    real(rkind), parameter :: b3_a80    = 0.9919620515685288!1.0!real(9.9952D-1, rkind)
    real(rkind), parameter :: b3_b80    = 0.6613080343790192!0.49875!0.9975!0.9985!0.9995!real(6.6656D-1, rkind) ! Already divided by factor of 2
    real(rkind), parameter :: b3_c80    = 0.1653270085947548!0.0!real( 1.668D-1, rkind) ! Already divided by factor of 2 
   
    ! Fourth point
    real(rkind), parameter :: b4_alpha80= 0.6568508886003703!0.49875!0.9975!0.9985!0.9995!real(6.6624D-1, rkind)
    real(rkind), parameter :: b4_beta80 = 0.16862982227992587!0.0!real(1.6688D-1, rkind)
    real(rkind), parameter :: b4_a80    = 0.9926381664502778!1.0!real(9.9968D-1, rkind)
    real(rkind), parameter :: b4_b80    = 0.6623722637626621!0.49875!0.9975!0.9985!0.9995!real(6.6652D-1, rkind) ! Already divided by factor of 2 
    real(rkind), parameter :: b4_c80    = 0.1664212722150092!0.0!real(1.6672D-1, rkind) ! Already divided by factor of 2 
    real(rkind), parameter :: b4_d80    = 0.36809167748611937D-3!0.0!real( 4.0D-5  , rkind) ! Already divided by factor of 2 
...
contains
    ...
    function init(this, n_, periodic_) result(ierr)
        ...
        if (periodic_) then 
            ...
            ! Compute LU matrix
            if (n_ .GE. 10) then
                call this%ComputeLU(this%LU,beta80,alpha80,one,alpha80,beta80)
            else if (n_ == 1) then
                this%LU = one
            else
                ierr = 7
    ...
    subroutine ComputePenta(this,penta,bc1,bcn)
        ...
        associate (bt   => penta(:,1), b   => penta(:,2), d => penta(:,3),  &
            ...
            case(0)
                ...
                bt(this%n-1) = zero
                b (this%n-1) = b2_alpha80
                d (this%n-1) = one
                a (this%n-1) = b2_alpha80
                at(this%n-1) = zero

                bt(this%n-2) = b3_beta80
                b (this%n-2) = b3_alpha80
                d (this%n-2) = one
                a (this%n-2) = b3_alpha80
                at(this%n-2) = b3_beta80

                bt(this%n-3) = b4_beta80
                b (this%n-3) = b4_alpha80
                d (this%n-3) = one
                a (this%n-3) = b4_alpha80
                at(this%n-3) = b4_beta80
            case(1)
                bt(this%n  ) = two*beta80
                b (this%n  ) = two*alpha80
                d (this%n  ) = one
                a (this%n  ) = zero 
                at(this%n  ) = zero
...
#include "CF80_files/SolveZPenta_common.F90"        
...
#include "CF80_files/SolveZPenta_common.F90"        
    ...
    pure subroutine ComputeXRHS(this, f, RHS, n2, n3, bc1, bcn)
        ...
        case (.TRUE.)
            do k=1,n3
                do j=1,n2
                    RHS(         1,j,k) = a80 * ( f(         1,j,k) )                     &
                                        + b80 * ( f(         2,j,k) + f(    this%n,j,k) ) &
                                        + c80 * ( f(         3,j,k) + f(  this%n-1,j,k) ) &
                                        + d80 * ( f(         4,j,k) + f(  this%n-2,j,k) ) &
                                        + e80 * ( f(         5,j,k) + f(  this%n-3,j,k) )
                    RHS(         2,j,k) = a80 * ( f(         2,j,k) )                     &
                                        + b80 * ( f(         3,j,k) + f(         1,j,k) ) &
                                        + c80 * ( f(         4,j,k) + f(    this%n,j,k) ) &
                                        + d80 * ( f(         5,j,k) + f(  this%n-1,j,k) ) &
                                        + e80 * ( f(         6,j,k) + f(  this%n-2,j,k) )
                    RHS(         3,j,k) = a80 * ( f(         3,j,k) )                     &
                                        + b80 * ( f(         4,j,k) + f(         2,j,k) ) &
                                        + c80 * ( f(         5,j,k) + f(         1,j,k) ) &
                                        + d80 * ( f(         6,j,k) + f(    this%n,j,k) ) &
                                        + e80 * ( f(         7,j,k) + f(  this%n-1,j,k) )
                    RHS(         4,j,k) = a80 * ( f(         4,j,k) )                     &
                                        + b80 * ( f(         5,j,k) + f(         3,j,k) ) &
                                        + c80 * ( f(         6,j,k) + f(         2,j,k) ) &
                                        + d80 * ( f(         7,j,k) + f(         1,j,k) ) &
                                        + e80 * ( f(         8,j,k) + f(    this%n,j,k) )
                    RHS(5:this%n-4,j,k) = a80 * ( f(5:this%n-4,j,k) )                     &
                                        + b80 * ( f(6:this%n-3,j,k) + f(4:this%n-5,j,k) ) &
                                        + c80 * ( f(7:this%n-2,j,k) + f(3:this%n-6,j,k) ) &
                                        + d80 * ( f(8:this%n-1,j,k) + f(2:this%n-7,j,k) ) &
                                        + e80 * ( f(9:this%n  ,j,k) + f(1:this%n-8,j,k) )
                    RHS(  this%n-3,j,k) = a80 * ( f(  this%n-3,j,k) )                     &
                                        + b80 * ( f(  this%n-2,j,k) + f(  this%n-4,j,k) ) &
                                        + c80 * ( f(  this%n-1,j,k) + f(  this%n-5,j,k) ) &
                                        + d80 * ( f(    this%n,j,k) + f(  this%n-6,j,k) ) &
                                        + e80 * ( f(         1,j,k) + f(  this%n-7,j,k) )
                    RHS(  this%n-2,j,k) = a80 * ( f(  this%n-2,j,k) )                     &
                                        + b80 * ( f(  this%n-1,j,k) + f(  this%n-3,j,k) ) &
                                        + c80 * ( f(    this%n,j,k) + f(  this%n-4,j,k) ) &
                                        + d80 * ( f(         1,j,k) + f(  this%n-5,j,k) ) &
                                        + e80 * ( f(         2,j,k) + f(  this%n-6,j,k) )
                    RHS(  this%n-1,j,k) = a80 * ( f(  this%n-1,j,k) )                     &
                                        + b80 * ( f(    this%n,j,k) + f(  this%n-2,j,k) ) &
                                        + c80 * ( f(         1,j,k) + f(  this%n-3,j,k) ) &
                                        + d80 * ( f(         2,j,k) + f(  this%n-4,j,k) ) &
                                        + e80 * ( f(         3,j,k) + f(  this%n-5,j,k) )
                    RHS(    this%n,j,k) = a80 * ( f(    this%n,j,k) )                     &
                                        + b80 * ( f(         1,j,k) + f(  this%n-1,j,k) ) &
                                        + c80 * ( f(         2,j,k) + f(  this%n-2,j,k) ) &
                                        + d80 * ( f(         3,j,k) + f(  this%n-3,j,k) ) &
                                        + e80 * ( f(         4,j,k) + f(  this%n-4,j,k) )
                end do
            end do
            ...
            do k = 1,n3
                do j = 1,n2
                    ...
                    case(0)
                        ...
                        RHS(         2,j,k) = b2_a80 * ( f(         2,j,k) )                     &
                                            + b2_b80 * ( f(         3,j,k) + f(         1,j,k) ) 
                        
                        RHS(         3,j,k) = b3_a80 * ( f(         3,j,k) )                     &
                                            + b3_b80 * ( f(         4,j,k) + f(         2,j,k) ) &
                                            + b3_c80 * ( f(         5,j,k) + f(         1,j,k) )

                        RHS(         4,j,k) = b4_a80 * ( f(         4,j,k) )                     &
                                            + b4_b80 * ( f(         5,j,k) + f(         3,j,k) ) &
                                            + b4_c80 * ( f(         6,j,k) + f(         2,j,k) ) &
                                            + b4_d80 * ( f(         7,j,k) + f(         1,j,k) ) 
                    case(1)
                        RHS(1,j,k) =    a80 * ( f(1,j,k) )            &
                                   +    b80 * ( f(2,j,k) + f(2,j,k) ) &
                                   +    c80 * ( f(3,j,k) + f(3,j,k) ) &
                    ...
                    select case(bcn)
                    case(0)
                        RHS(  this%n-3,j,k) = b4_a80 * ( f(  this%n-3,j,k) )                     &
                                            + b4_b80 * ( f(  this%n-2,j,k) + f(  this%n-4,j,k) ) &
                                            + b4_c80 * ( f(  this%n-1,j,k) + f(  this%n-5,j,k) ) &
                                            + b4_d80 * ( f(    this%n,j,k) + f(  this%n-6,j,k) ) 

                        RHS(  this%n-2,j,k) = b3_a80 * ( f(  this%n-2,j,k) )                     &
                                            + b3_b80 * ( f(  this%n-1,j,k) + f(  this%n-3,j,k) ) &
                                            + b3_c80 * ( f(    this%n,j,k) + f(  this%n-4,j,k) ) 

                        RHS(  this%n-1,j,k) = b2_a80 * ( f(  this%n-1,j,k) )                     &
                                            + b2_b80 * ( f(    this%n,j,k) + f(  this%n-2,j,k) ) 
                    ...
                    case(1)
                        RHS(this%n-3,j,k) =    a80 * ( f(this%n-3,j,k) )                   &
                                          ...
                                          +    c80 * ( f(this%n-1,j,k) + f(this%n-5,j,k) ) &
                                          +    d80 * ( f(this%n  ,j,k) + f(this%n-6,j,k) ) &
                                          +    e80 * ( f(this%n-1,j,k) + f(this%n-7,j,k) )
                        RHS(this%n-2,j,k) =    a80 * ( f(this%n-2,j,k) )                   &
                                          +    b80 * ( f(this%n-1,j,k) + f(this%n-3,j,k) ) &
                                          +    c80 * ( f(this%n  ,j,k) + f(this%n-4,j,k) ) &
                                          +    d80 * ( f(this%n-1,j,k) + f(this%n-5,j,k) ) &
                                          +    e80 * ( f(this%n-2,j,k) + f(this%n-6,j,k) )
                        RHS(this%n-1,j,k) =    a80 * ( f(this%n-1,j,k) )                   &
                                          +    b80 * ( f(this%n  ,j,k) + f(this%n-2,j,k) ) &
                                          +    c80 * ( f(this%n-1,j,k) + f(this%n-3,j,k) ) &
                                          +    d80 * ( f(this%n-2,j,k) + f(this%n-4,j,k) ) &
                                          +    e80 * ( f(this%n-3,j,k) + f(this%n-5,j,k) )
                        RHS(this%n  ,j,k) =    a80 * ( f(this%n  ,j,k) )                   &
                                          +    b80 * ( f(this%n-1,j,k) + f(this%n-1,j,k) ) &
                                          +    c80 * ( f(this%n-2,j,k) + f(this%n-2,j,k) ) &
                                          +    d80 * ( f(this%n-3,j,k) + f(this%n-3,j,k) ) &
                                          +    e80 * ( f(this%n-4,j,k) + f(this%n-4,j,k) )
                    case(-1)
                        RHS(this%n-3,j,k) =    a80 * ( f(this%n-3,j,k) )                   &
                                          +    b80 * ( f(this%n-2,j,k) + f(this%n-4,j,k) ) &
                                          +    c80 * ( f(this%n-1,j,k) + f(this%n-5,j,k) ) &
    ...
    pure subroutine ComputeYRHS(this, f, RHS, n1, n3, bc1, bcn) 
        ...
        case (.TRUE.)
            do k=1,n3
                RHS(:,         1,k) = a80 * ( f(:,         1,k) )                     &
                                    + b80 * ( f(:,         2,k) + f(:,    this%n,k) ) &
                                    + c80 * ( f(:,         3,k) + f(:,  this%n-1,k) ) &
                                    + d80 * ( f(:,         4,k) + f(:,  this%n-2,k) ) &
                                    + e80 * ( f(:,         5,k) + f(:,  this%n-3,k) )
                RHS(:,         2,k) = a80 * ( f(:,         2,k) )                     &
                                    + b80 * ( f(:,         3,k) + f(:,         1,k) ) &
                                    + c80 * ( f(:,         4,k) + f(:,    this%n,k) ) &
                                    + d80 * ( f(:,         5,k) + f(:,  this%n-1,k) ) &
                                    + e80 * ( f(:,         6,k) + f(:,  this%n-2,k) )
                RHS(:,         3,k) = a80 * ( f(:,         3,k) )                     &
                                    + b80 * ( f(:,         4,k) + f(:,         2,k) ) &
                                    + c80 * ( f(:,         5,k) + f(:,         1,k) ) &
                                    + d80 * ( f(:,         6,k) + f(:,    this%n,k) ) &
                                    + e80 * ( f(:,         7,k) + f(:,  this%n-1,k) )
                RHS(:,         4,k) = a80 * ( f(:,         4,k) )                     &
                                    + b80 * ( f(:,         5,k) + f(:,         3,k) ) &
                                    + c80 * ( f(:,         6,k) + f(:,         2,k) ) &
                                    + d80 * ( f(:,         7,k) + f(:,         1,k) ) &
                                    + e80 * ( f(:,         8,k) + f(:,    this%n,k) )
                RHS(:,5:this%n-4,k) = a80 * ( f(:,5:this%n-4,k) )                     &
                                    + b80 * ( f(:,6:this%n-3,k) + f(:,4:this%n-5,k) ) &
                                    + c80 * ( f(:,7:this%n-2,k) + f(:,3:this%n-6,k) ) &
                                    + d80 * ( f(:,8:this%n-1,k) + f(:,2:this%n-7,k) ) &
                                    + e80 * ( f(:,9:this%n  ,k) + f(:,1:this%n-8,k) )
                RHS(:,  this%n-3,k) = a80 * ( f(:,  this%n-3,k) )                     &
                                    + b80 * ( f(:,  this%n-2,k) + f(:,  this%n-4,k) ) &
                                    + c80 * ( f(:,  this%n-1,k) + f(:,  this%n-5,k) ) &
                                    + d80 * ( f(:,    this%n,k) + f(:,  this%n-6,k) ) &
                                    + e80 * ( f(:,         1,k) + f(:,  this%n-7,k) )
                RHS(:,  this%n-2,k) = a80 * ( f(:,  this%n-2,k) )                     &
                                    + b80 * ( f(:,  this%n-1,k) + f(:,  this%n-3,k) ) &
                                    + c80 * ( f(:,    this%n,k) + f(:,  this%n-4,k) ) &
                                    + d80 * ( f(:,         1,k) + f(:,  this%n-5,k) ) &
                                    + e80 * ( f(:,         2,k) + f(:,  this%n-6,k) )
                RHS(:,  this%n-1,k) = a80 * ( f(:,  this%n-1,k) )                     &
                                    + b80 * ( f(:,    this%n,k) + f(:,  this%n-2,k) ) &
                                    + c80 * ( f(:,         1,k) + f(:,  this%n-3,k) ) &
                                    + d80 * ( f(:,         2,k) + f(:,  this%n-4,k) ) &
                                    + e80 * ( f(:,         3,k) + f(:,  this%n-5,k) )
                RHS(:,    this%n,k) = a80 * ( f(:,    this%n,k) )                     &
                                    + b80 * ( f(:,         1,k) + f(:,  this%n-1,k) ) &
                                    + c80 * ( f(:,         2,k) + f(:,  this%n-2,k) ) &
                                    + d80 * ( f(:,         3,k) + f(:,  this%n-3,k) ) &
                                    + e80 * ( f(:,         4,k) + f(:,  this%n-4,k) )
            end do
        case (.FALSE.)
            ...
            do k = 1,n3
                ...
                case(0)
                    ...
                    RHS(:,         2,k) = b2_a80 * ( f(:,         2,k) )                     &
                                        + b2_b80 * ( f(:,         3,k) + f(:,         1,k) ) 
                    
                    RHS(:,         3,k) = b3_a80 * ( f(:,         3,k) )                     &
                                        + b3_b80 * ( f(:,         4,k) + f(:,         2,k) ) &
                                        + b3_c80 * ( f(:,         5,k) + f(:,         1,k) )

                    RHS(:,         4,k) = b4_a80 * ( f(:,         4,k) )                     &
                                        + b4_b80 * ( f(:,         5,k) + f(:,         3,k) ) &
                                        + b4_c80 * ( f(:,         6,k) + f(:,         2,k) ) &
                                        + b4_d80 * ( f(:,         7,k) + f(:,         1,k) ) 
                case(1)
                ...
                select case(bcn)
                case(0)
                    RHS(:,  this%n-3,k) = b4_a80 * ( f(:,  this%n-3,k) )                     &
                                        + b4_b80 * ( f(:,  this%n-2,k) + f(:,  this%n-4,k) ) &
                                        + b4_c80 * ( f(:,  this%n-1,k) + f(:,  this%n-5,k) ) &
                                        + b4_d80 * ( f(:,    this%n,k) + f(:,  this%n-6,k) ) 

                    RHS(:,  this%n-2,k) = b3_a80 * ( f(:,  this%n-2,k) )                     &
                                        + b3_b80 * ( f(:,  this%n-1,k) + f(:,  this%n-3,k) ) &
                                        + b3_c80 * ( f(:,    this%n,k) + f(:,  this%n-4,k) ) 

                    RHS(:,  this%n-1,k) = b2_a80 * ( f(:,  this%n-1,k) )                     &
                                        + b2_b80 * ( f(:,    this%n,k) + f(:,  this%n-2,k) ) 
                ...
                case(1)
                    RHS(:,this%n-3,k) =    a80 * ( f(:,this%n-3,k) )                   &
                                      ...
                                      +    c80 * ( f(:,this%n-1,k) + f(:,this%n-5,k) ) &
                                      +    d80 * ( f(:,this%n  ,k) + f(:,this%n-6,k) ) &
                                      +    e80 * ( f(:,this%n-1,k) + f(:,this%n-7,k) )
                    RHS(:,this%n-2,k) =    a80 * ( f(:,this%n-2,k) )                   &
                                      +    b80 * ( f(:,this%n-1,k) + f(:,this%n-3,k) ) &
                                      +    c80 * ( f(:,this%n  ,k) + f(:,this%n-4,k) ) &
                                      +    d80 * ( f(:,this%n-1,k) + f(:,this%n-5,k) ) &
                                      +    e80 * ( f(:,this%n-2,k) + f(:,this%n-6,k) )
                    RHS(:,this%n-1,k) =    a80 * ( f(:,this%n-1,k) )                   &
                                      +    b80 * ( f(:,this%n  ,k) + f(:,this%n-2,k) ) &
                                      +    c80 * ( f(:,this%n-1,k) + f(:,this%n-3,k) ) &
                                      +    d80 * ( f(:,this%n-2,k) + f(:,this%n-4,k) ) &
                                      +    e80 * ( f(:,this%n-3,k) + f(:,this%n-5,k) )
                    RHS(:,this%n  ,k) =    a80 * ( f(:,this%n  ,k) )                   &
                                      +    b80 * ( f(:,this%n-1,k) + f(:,this%n-1,k) ) &
                                      +    c80 * ( f(:,this%n-2,k) + f(:,this%n-2,k) ) &
                                      +    d80 * ( f(:,this%n-3,k) + f(:,this%n-3,k) ) &
                                      +    e80 * ( f(:,this%n-4,k) + f(:,this%n-4,k) )
                case(-1)  
                    RHS(:,this%n-3,k) =    a80 * ( f(:,this%n-3,k) )                   &
                                      +    b80 * ( f(:,this%n-2,k) + f(:,this%n-4,k) ) &
                                      +    c80 * ( f(:,this%n-1,k) + f(:,this%n-5,k) ) &
...
#include "CF80_files/ComputeZRHS_common.F90"    
