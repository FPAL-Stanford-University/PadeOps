...
contains
    ...
    subroutine doTemporalStuff(gp, simid)
        ...
        if (mod(gp%step,nt_print2screen) == 0) then
            ...
            elseif (simid == 2) then
               call message(1,"Mean   u for HIT:", &
                 p_sum(sum(gp%u))/real(gp%nx*gp%ny*gp%nz,rkind))
               call message(1,"Mean   v for HIT:", &
                 p_sum(sum(gp%v))/real(gp%nx*gp%ny*gp%nz,rkind))
               call message(1,"Mean   w for HIT:", &
                 p_sum(sum(gp%wC))/real(gp%nx*gp%ny*gp%nz,rkind))
