...
#include "diurnal_concurrent_files/initialize.F90"       
#include "diurnal_concurrent_files/temporalHook.F90"  

program diurnal_concurrent
    use mpi
    ...
    call setup_diurnalBCs(primary_inputFile) 
    ...
    ! check boundary conditions associated with the diurnal cycle problem 
    ...
    do while (primary%tsim < primary%tstop) 
       ! Get diurnal cycle boundary conditions
       call get_diurnalBCs(precursor%tsim, precursor%G_geostrophic, precursor%wTh_surf, precursor%G_alpha)
       call get_diurnalBCs(primary%tsim, primary%G_geostrophic, primary%wTh_surf, primary%G_alpha)
       ! Get the time step
