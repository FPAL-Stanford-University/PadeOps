module getBudgetTerms_parameters
    ...
    integer :: tStatStart = 210000, tStatEnd = 219770, dtStats = 10, dtStatsDump = 219770
...
subroutine meshgen_wallM(decomp, dx, dy, dz, mesh, inputfile)
    ...
    integer :: tStatStartIn, tStatEndIn, dtStatsIn, dtStatsDumpIn 
    real(rkind)  :: Lx = one, Ly = one, Lz = one
    namelist /PBLINPUT/ Lx, Ly, Lz, z0init
    namelist /STATSCOMPINPUT/ tStatStartIn, tStatEndIn, dtStatsIn, dtStatsDumpIn 

    ioUnit = 11
    open(unit=ioUnit, file=trim(inputfile), form='FORMATTED')
    read(unit=ioUnit, NML=PBLINPUT)
    read(unit=ioUnit, NML=STATSCOMPINPUT)
    close(ioUnit)    
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        tStatStart = tStatStartIn; tStatEnd = tStatEndIn; dtStats = dtStatsIn; dtStatsDump = dtStatsDumpIn 
