module MixedConvection_parameters
    ...
    real(rkind), parameter :: timeDim = 5.d0 ! Not used in any way  

end module     

subroutine meshgen_wallM(decomp, dx, dy, dz, mesh, inputfile)
    use MixedConvection_parameters    
    ...
    read(unit=ioUnit, NML=MixedConvectionInputs)
    close(ioUnit)    
...
subroutine initfields_wallM(decompC, decompE, inputfile, mesh, fieldsC, fieldsE)
    ...
    use MixedConvection_IO, only: get_perturbations 
    ...
    kappa = (Ra**(1.d0/4.d0))/sqrt(2.d0)
    E = -2.d0*kappa*sqrt(Ra)*(cosh(2*kappa)  + cos(2*kappa))/(sinh(2*kappa) - sin(2*kappa))
    u  = -(E/sqrt(Ra))*(sinh(kappa*(1+z))*sin(kappa*(1-z)) + sin(kappa*(1+z))*sinh(kappa*(1-z)))/(cosh(2*kappa) +  cos(2*kappa))
    v  =  zero  
    wC = zero
    T = (E/Ra)*(1 - (cosh(kappa*(1+z))*cos(kappa*(1-z)) + cos(kappa*(1+z))*cosh(kappa*(1-z)))/(cosh(2*kappa) +  cos(2*kappa)));
    ...
    allocate(derW)
    dz = z(1,1,2) - z(1,1,1)
    call derW%init(decompC%zsz(3), dz, isTopEven = .true., isBotEven = .true., &
