module ShearLayerSin_data
    ...
    real(rkind) :: Ly = 2.0, Lx = 1.357168026, interface_init = 10d-3, kwave = 4.0_rkind, ksize = 10d0, etasize = 0.5d0, delta_d = 0.0125D0, delta = 0.0125D0, delta_rho = 0.0125D0 
...
subroutine initfields(decomp,der,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    integer :: ioUnit,i,iy
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp, dum, eta, eta2,eta3
	...
	mix%material(2)%Ys = one - mix%material(1)%Ys ! Enforce sum to unity
       ...
       ! kwave = ksize*delta
        ...
        v   = v_disturb*exp(-abs((eta3/(1.5*delta))))*(alpha4*sin(alpha4*x))  !+alpha4*sin(alpha4*(2/3)*y)+alpha4*sin(alpha4/3*y)+alpha4*sin(alpha4*(3/4)*y)+ alpha4*sin(alpha4/2*y))
        where(eta2 .ge. 0)
           u = v0_2*erf(eta2/delta) + v_disturb*-1.0/delta*3/2*exp(-abs((eta3/(1.5*delta))))*cos(alpha4*x) !3/2*cos(alpha4*(2/3)*y)+3*cos(alpha4/3*y)+cos(alpha4*y)+ 4/3*cos(alpha4*(3/4)*y)+2*cos(alpha4/2*y) )   ! + U0*(1 - erf(eta/delta_d))
        elsewhere(eta2 .lt. 0 )
           u = v0*erf(eta2/delta) + v_disturb*-1.0/delta*3/2*exp(-abs((eta3/(1.5*delta))))*cos(alpha4*x) !(3/2*cos(alpha4*(2/3)*y)+3*cos(alpha4/3*y)+cos(alpha4*y)+ 4/3*cos(alpha4*(3/4)*y)+2*cos(alpha4/2*y))  !!+ !U0*(1 + erf(eta/delta_d))
        endwhere
        w   = zero
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/ShearLayerSin_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/ShearLayerSin_statistics.dat"
