module RTCollis_data
    ...
    real(rkind) :: Ly = 3.0, Lx = 3.0,interface_init = 10d-3, kwave = 4.0_rkind, ksize = 10d0, etasize = 0.5d0, delta_d = 0.0125D0, delta = 0.0125D0, delta_rho = 0.0125D0 
...
!        if(adjustRgas) Rgas_2 = Rgas * (p_amb+p_infty_2)/(p_amb+p_infty)*rho_0/rho_0_2
        ...
        delta_rho = Nrho * dx * 0.75d0*2d0 !converts from Nrho to approximate thickness of erf profile
        eta2 = y-0.1d0*cos(2.0_rkind*pi*x) - 2d0
        ...
        tmp = (half ) * ( one + tanh( (eta2)/(delta_rho) ) )
       ...
       ! mix%material(2)%Ys = one - mix%material(1)%Ys 
        ...
        p(:,ny,:) = p_amb
        do j = ny-1,1,-1
        p(:,j,:) = p(:,j+1,:) +0.5*( rho(:,j,:)+rho(:,j+1,:))*(y(:,j+1,:)-y(:,j,:))*9.81d0 
        enddo
        mix%material(1)%p = p
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,pthick,rhothick,uthick,Ysthick,VFthick,Ys_wiggle,VF_wiggle,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/RTCollis_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/RTCollis_statistics.dat"
