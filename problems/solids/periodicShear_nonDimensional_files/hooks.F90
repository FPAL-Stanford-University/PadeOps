module periodicShear_nonDimensional_data
    ...
    real(rkind) :: rho_0 = one, tau_0 = 1d-10
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    use periodicShear_nonDimensional_data
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    real(rkind) :: u_amp, u_advect, v_amp, p_amb, u_scale, p_scale, rho_scale, t_scale 

    integer :: nx,ny,nz
    nx = size(mesh,1); ny = size(mesh,2); nz = size(mesh,3)

    namelist /PROBINPUT/  p_infty, Rgas, gamma, mu, rho_0, tau_0, &
                          plastic, explPlast, yield, u_amp, v_amp, u_advect, p_amb, &
    ...
    !Non-dimensionalize
    !pressures/stresses    
    p_scale = gamma * p_infty + (mu * 4.0d0/3.0d0 )
    p_infty = p_infty / p_scale  
    mu      = mu      / p_scale
    yield   = yield   / p_scale
    p_amb   = p_amb   / p_scale
    ...
    !densities
    rho_scale = rho_0    
    rho_0 = rho_0/rho_scale    

    !velocities
    u_scale = sqrt(p_scale/rho_scale)    
    u_advect = u_advect / u_scale       
    u_amp    = u_amp    / u_scale 
    v_amp    = v_amp    / u_scale 
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        if (nrank == 0) then
            ...
            write(*,'(3(a,e12.5))') 'p_amb = ', p_amb, ', gam  = ', gamma
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/periodicShear_nonDimensional_"//trim(str)//"_", vizcount, ".dat"
