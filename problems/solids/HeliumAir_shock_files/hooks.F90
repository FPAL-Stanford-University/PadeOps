module HeliumAir_shock_data
    ...
    real(rkind) :: Ly = two, Lx = two, interface_init = 0.75_rkind, shock_init = 0.6_rkind, kwave = 4.0_rkind, thick2 
...
subroutine initfields(decomp,der,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    associate(   pEvolve => fields(:,:,:,pEvolve_index),rho    => fields(:,:,:, rho_index),  u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        shock_init = -0.8 !interface_init !- 1.0_rkind  ! (10*thick) grid points away from the interface
        ...
        tmp = 0.5*(1 + tanh( (x + 0.2)/(thick2*dx) ) ) !dum = tmp

        u   = 26.59*dum + (-0.5)*(one-dum)
        v   = zero
        w   = zero
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/HeliumAir_shock_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/HeliumAir_shock_statistics.dat"
