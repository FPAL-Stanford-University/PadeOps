module Impact_groove_data
    ...
    real(rkind) :: Ly = two, Lx = two, interface_init = 0.0_rkind, shock_init = 0.6_rkind, kwave = 4.0_rkind, kwave_i = 2.0_rkind
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    integer :: ioUnit
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp, dum, int_shape, flyer_shape, target_shape, yr, perturbations
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        yr = (y - 0.5 * Ly) / (0.5 * Ly) ! yr is reference y location. yr [-1,1)
        kwave_i = 2.0_rkind

        target_shape  = 0.5 + 0.5*erf( (x - (Lx - (l0 + 0.5 * Ly * yr * tan(theta)))) / (thick*dx) )
        flyer_shape   = 0.5 * ( erf((x+0.2)/(thick*dx)) - erf((x-0.0)/(thick*dx)) )
        tmp = target_shape + flyer_shape
...
subroutine hook_mixture_source(decomp,mesh,fields,mix,tsim,rhs)
    ...
    if (acc_force_mode) then
        ...
        if (tsim .LT. time_acc) then
            rhs(:,:,:, mom_index  ) = rhs(:,:,:, mom_index)   + (x<0) * rho * mix%material(1)%VF * acc_x
            rhs(:,:,:, mom_index+1) = rhs(:,:,:, mom_index+1) + (x<0) * rho * mix%material(1)%VF * acc_y
            rhs(:,:,:, TE_index)    = rhs(:,:,:, TE_index)    + (x<0) * rho * mix%material(1)%VF * ((acc_x * u) + (acc_y * v))
        endif
    endif
