...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        if ( sharp ) then
            tmp = half * ( erf( (x-0.2_rkind+0.05_rkind)/(thick*dx) ) - erf( (x-0.2_rkind-0.05_rkind)/(thick*dx) ) )
        else
            tmp = exp(-((x-0.3_rkind)/0.1_rkind)**2)
        end if
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
        ...
        write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/CylNoh_"//trim(str)//"_", vizcount, ".dat"
...
subroutine hook_timestep(decomp,mesh,fields,mix,step,tsim)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
        ...
        call message("Max. g11-1",P_MAXVAL(mix%material(1)%g11))
        call message("Min. g11-1",P_MINVAL(mix%material(1)%g11))
        call message("Max. g11-2",P_MAXVAL(mix%material(2)%g11))
        call message("Min. g11-2",P_MINVAL(mix%material(2)%g11))
    end associate
end subroutine
