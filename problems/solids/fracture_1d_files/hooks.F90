module fracture_1d_data
    ...
    real(rkind) :: u_L = 0d0, u_R = 0d0, v_L = 0d0, v_R =0d0, rho_BC = one

    real(rkind) :: interface_init = 0.5, thick=0.01, minVF = 1.0d-16
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/fracture_1d_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/fracture_1d_statistics.dat"
