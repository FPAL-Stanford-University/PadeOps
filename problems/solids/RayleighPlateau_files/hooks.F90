module ShearLayerOld_data
    ...
    real(rkind) :: Ly = 8d-3, Lx = 9.02d-3,interface_init = 10d-3, kwave = 4.0_rkind, ksize = 10d0, etasize = 0.5d0, delta_d = 0.0125D0, delta = 0.0125D0, delta_rho = 0.0125D0 
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1 - 1   + i - 1, rkind ) * dx -9.02d-3/2.0
                    y(i,j,k) = real( iy1 - 1  + j - 1, rkind ) * dy - 4d-3
...
!   dx1 = 2*pi/128
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        delta_rho = Nrho * dx * (2)**0.5_rkind !converts from Nrho to approximate thickness of erf profile
...
!        eta2 = yphys !eta !-delta_rho
        ...
        eta =( 1 + 0.5*cos(2*pi*x/9.02d-3) )*1d-3
        mix%material(1)%p  = p_amb
        tmp = (half ) * ( one - tanh( (y-eta)/(delta_rho) ) ) + (half ) * ( one + tanh( (y+eta)/(delta_rho) ) ) - 1
...
!        yphys = Lr*yphys
        ...
        where(y .LE. -3.5d-3)
           sponge(:,:,:,1) = sigma1*( (y + 3.5d-3)/5d-4)**2.0
        elsewhere
           sponge(:,:,:,1) = 0
        endwhere

        where(y .GE. 3.5d-3)
           sponge(:,:,:,2) = sigma1*( (y- 3.5d-3)/5d-4)**2.0 
        elsewhere
           sponge(:,:,:,2) = 0
        endwhere
