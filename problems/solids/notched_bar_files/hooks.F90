module notched_bar_data
    ...
    real(rkind) :: Lx = 12.0d-2 , Ly = 4.0d-2
...
function myStep_plane(decomp,x_,y_,x0,y0,dx_,dy_,thick_) result(res)
    use kind_parameters,  only: rkind
    ...
    real(rkind), dimension(:,:,:), intent(in)  :: x_, y_, x0, y0 
    real(rkind), intent(in) :: dx_, dy_, thick_
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: res
    res = 0.5d0*(1.0d0+erf( (x_-x0)/(dx_*thick_)))*(1.0d0+erf( (y_-y0)/(dy_*thick_)))
end
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: bar, tri, tri1, tri2, notch, ones
    real(rkind) :: m, b1, b2 

    integer :: nx,ny,nz
    nx = size(mesh,1); ny = size(mesh,2); nz = size(mesh,3)

    namelist /PROBINPUT/  p_amb, interface_init, thick, minVF, &
                          u_jump, v_jump, &
                          lBar, wBar, bNotch, hNotch, &

                          p_infty, Rgas, gamma, mu, rho_0, yield, plastic, explPlast, &
                          melt_t, melt_c, kos_b, kos_t, kos_h, kos_g, kos_m, &
                          kos_q, kos_f, kos_alpha, kos_beta, kos_e, kos_sh, &    
                          eta_det_ge, eta_det_gp, eta_det_gt, diff_c_ge, diff_c_gp, diff_c_gt, & 

                          p_infty_2, Rgas_2, gamma_2, mu_2, rho_0_2, yield2, plastic2, explPlast2, &
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        ! adjust ambient properties for p-T equilibrium 
        Rgas_2 = Rgas * (p_amb+p_infty_2)/(p_amb+p_infty)*rho_0/rho_0_2
        
        ! Set materials (including gamma, Pinf, gamma, , rho0, mu, yieldm and tau0)
        call mix%set_material(1,stiffgas(gamma  ,Rgas  ,p_infty  ),sep1solid(rho_0  ,mu  ,yield,1.0D-14,eta_det_ge,eta_det_gp,eta_det_gt,diff_c_ge,diff_c_gp,diff_c_gt,melt_t,melt_c,kos_b,kos_t,kos_h,kos_g,kos_m,kos_q,kos_f,kos_alpha,kos_beta,kos_e,kos_sh,nx,ny,nz)) !mca: see Sep1SolidEOS.F90 "init"
        call mix%set_material(2,stiffgas(gamma_2,Rgas_2,p_infty_2),sep1solid(rho_0_2,mu_2,yield2,1.0D-14,eta_det_ge_2,eta_det_gp_2,eta_det_gt_2,diff_c_ge_2,diff_c_gp_2,diff_c_gt_2,melt_t2,melt_c2,kos_b2,kos_t2,kos_h2,kos_g2,kos_m2,kos_q2,kos_f2,kos_alpha2,kos_beta2,kos_e2,kos_sh2,nx,ny,nz))
        ...
        if (nrank == 0) then
            ...
            write(*,'(3(a,e12.5))') '=========================================='
            write(*,'(3(a,e12.5))') 'Main material properties'
            write(*,'(3(a,e12.5))') '=========================================='
            write(*,'(3(a,e12.5))') '    mu    = ', mu,    ', Rgas  = ', Rgas,   ', rho_0   = ', rho_0
            write(*,'(3(a,e12.5))') '    yield = ', yield, ', p_infty = ', p_infty
            write(*,'(3(a,e12.5))') '    gam   = ', gamma, ', p = ', p_amb
            write(*,'(3(a,e12.5))') '=========================================='
            write(*,'(3(a,e12.5))') '=========================================='
            write(*,'(3(a,e12.5))') 'Ambient properties'
            write(*,'(3(a,e12.5))') '=========================================='
            write(*,'(3(a,e12.5))') '    mu    = ', mu_2,    ', Rgas  = ', Rgas_2,   ', rho_0   = ', rho_0_2
            write(*,'(3(a,e12.5))') '    yield = ', yield2, ', p_infty = ', p_infty_2
            write(*,'(3(a,e12.5))') '    gam   = ', gamma_2, ', p = ', p_amb
            write(*,'(3(a,e12.5))') '=========================================='
            write(*,'(3(a,e12.5))') '=========================================='
        end if

        !Generate notched bar shape                
        ones = 1.0d0
        bar = myStep_plane(decomp,x,y,ones*(Lx/2.0d0-lBar/2.0d0),y,dx,dy,thick);
        bar = bar*myStep_plane(decomp,-x,y,ones*(-Lx/2.0d0-lBar/2.0d0),y,dx,dy,thick);
        bar = bar*myStep_plane(decomp,x,y,x,ones*(Ly/2.0d0-wBar/2.0d0),dx,dy,thick);
        bar = bar*myStep_plane(decomp,x,-y,x,ones*(-Ly/2.0d0-wBar/2.0d0),dx,dy,thick);
        
        m = hNotch/(bNotch/2.0d0);
        b1 = Ly/2.0d0+(wBar/2.0d0-hNotch)-m*Lx/2.0d0;
        b2 = Ly/2.0d0+(wBar/2.0d0-hNotch)+m*Lx/2.0d0;
        tri1 = myStep_plane(decomp,x,y,x,m*x+b1,dx,dy,thick);
        tri1 = tri1*myStep_plane(decomp,x,y,x,-m*x+b2,dx,dy,thick);
        
        b1 = Ly/2.0d0-(wBar/2.0d0-hNotch)-m*Lx/2.0d0;
        b2 = Ly/2.0d0-(wBar/2.0d0-hNotch)+m*Lx/2.0d0;
        tri2 = myStep_plane(decomp,-x,-y,-x,-m*x-b1,dx,dy,thick);
        tri2 = tri2*myStep_plane(decomp,-x,-y,-x,m*x-b2,dx,dy,thick);
        ...
        !set volume fraction, density, and mass fraction
        mix%material(1)%VF = (one - 2.0d0*minVF) * bar + minVF
        mix%material(2)%VF = one - mix%material(1)%VF
        rho = mix%material(1)%VF*rho_0 + mix%material(2)%VF*rho_0_2 
        ...
        !set mixture pressure, kinematic tensors
        DO ind=1,mix%ns
            mix%material(ind)%p = p_amb

            mix%material(ind)%g11  =  one; mix%material(ind)%g12  = zero; mix%material(ind)%g13  = zero 
            mix%material(ind)%g21  = zero; mix%material(ind)%g22  =  one; mix%material(ind)%g23  = zero 
            mix%material(ind)%g31  = zero; mix%material(ind)%g32  = zero; mix%material(ind)%g33  = one 

            mix%material(ind)%gp11 =  one; mix%material(ind)%gp12 = zero; mix%material(ind)%gp13 = zero 
            mix%material(ind)%gp21 = zero; mix%material(ind)%gp22 =  one; mix%material(ind)%gp23 = zero 
            mix%material(ind)%gp31 = zero; mix%material(ind)%gp32 = zero; mix%material(ind)%gp33 = one 

            mix%material(ind)%gt11 =  one; mix%material(ind)%gt12 = zero; mix%material(ind)%gt13 = zero 
            mix%material(ind)%gt21 = zero; mix%material(ind)%gt22 =  one; mix%material(ind)%gt23 = zero 
            mix%material(ind)%gt31 = zero; mix%material(ind)%gt32 = zero; mix%material(ind)%gt33 = one 
        ENDDO
       
        if ( mix%use_gTg ) then
            CONTINUE
        else
            CALL GracefulExit("This test problems assume ge-Gp formulation, which required use_gTg=.TRUE. ",928)
        endif
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/notched_bar_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/notched_bar_statistics.dat"
      ...
      mix%material(1)%pe = zero
        ...
        if(decomp%yen(1)==decomp%xsz(1)) then
          if(x_bc(2)==0) then
              ...
              mix%material(1)%Ys ( nx,:,:) = rho_0 * mix%material(1)%VF( nx,:,:) / rho( nx,:,:)
              mix%material(2)%Ys ( nx,:,:) = one   - mix%material(1)%Ys( nx,:,:)
          endif
        endif
        ...
        do i=1,4 
            ...
            do j = 1,9
                DO ind=1,mix%ns
                    tmp = mix%material(ind)%g(:,:,:,j)
                    call filter3D(decomp,mygfil,tmp,1,x_bc,y_bc,z_bc)
                    mix%material(ind)%g(:,:,:,j) = mix%material(ind)%g(:,:,:,j) + dum*(tmp - mix%material(ind)%g(:,:,:,j))

                    tmp = mix%material(ind)%g_t(:,:,:,j)
                    call filter3D(decomp,mygfil,tmp,1,x_bc,y_bc,z_bc)
                    mix%material(ind)%g_t(:,:,:,j) = mix%material(ind)%g_t(:,:,:,j) + dum*(tmp - mix%material(ind)%g_t(:,:,:,j))

                    tmp = mix%material(ind)%g_p(:,:,:,j)
                    call filter3D(decomp,mygfil,tmp,1,x_bc,y_bc,z_bc)
                    mix%material(ind)%g_p(:,:,:,j) = mix%material(ind)%g_p(:,:,:,j) + dum*(tmp - mix%material(ind)%g_p(:,:,:,j))
                ENDDO
            end do
        end do
