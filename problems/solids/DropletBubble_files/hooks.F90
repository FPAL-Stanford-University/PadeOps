...
!
        ...
        elsewhere
           ...
           tmp = one - half*(one- erf((0.425_rkind*0.425_rkind - (x)*(x) -(y)*(y))/(3_rkind*thick*dx) ) ) 
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/DropletBubble_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/DropletBubble_statistics.dat"
