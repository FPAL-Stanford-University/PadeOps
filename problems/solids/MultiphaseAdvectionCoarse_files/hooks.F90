module MultiphaseAdvectionCoarse_data
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    use MultiphaseAdvectionCoarse_data
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1     + i - 1, rkind ) * dx + 0.25 !- two  ! x \in (-2,4]
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        tmp = half * ( one - erf((25.0_rkind/31684.0_rkind - (x-interface_init)*(x-interface_init) - (y-0.5_rkind)*(y-0.5_rkind))/(thick*dx) ) )
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/MultiphaseAdvectionCoarse_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/MultiphaseAdvectionCoarse_statistics.dat"
