module Multispecies_shock_data
    ...
    real(rkind) :: yield = one, yield2 = one, eta0k = 0.4_rkind, eta0x = 0.2_rkind
    ...
    !real(rkind) :: Ly = one, Lx = 20.0_rkind, interface_init = 0.0_rkind, shock_init = -6.0_rkind, fluc_init=-5.0, kwave = 4.0_rkind, xwave = 4.0
    real(rkind) :: Ly = one, Lx = 6.0_rkind, interface_init = 0.75_rkind, shock_init = 0.6_rkind, fluc_init=0.0, kwave = 4.0_rkind, xwave = 4.0
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1     + i - 1, rkind ) * dx - 10.0_rkind !two  ! x \in (-2,4]
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    namelist /PROBINPUT/  p_infty, Rgas, gamma, mu, rho_0, p_amb, thick, minVF, rhoRatio, pRatio, &
                          ...
                          plastic2, explPlast2, yield2, interface_init, kwave, xwave,&
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), rho => fields(:,:,:,rho_index), &
        ...
        !mod for ShuOsher: already defined
        ...
        dum2 = eta0x/(2.0_rkind*pi*xwave)*sin(2.0_rkind*xwave*pi*x)
        ! do i=1,decomp%ysz(1)
        !    do j=1,decomp%ysz(2)
        !       do k=1,decomp%ysz(3)
        !          if(x(i,j,k).le.fluc_init) then
        !             dum2(i,j,k) = 0.0_rkind
        !          endif
        !       enddo
        !    enddo
        ! enddo
        ...
        !mix%material(1)%gt11 = (rho2*dum + rho1*(one-dum))/rho_0 * dum2
