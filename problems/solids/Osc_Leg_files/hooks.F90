...
subroutine initfields(decomp,der,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    integer :: ioUnit
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp2, tmp, dum, eta,rad,Rp
...
!    call MPI_Comm_SIZE(MPI_COMM_WORLD, totalproc, ierr)
  ...
  if (nrank == 0) then
	...
	!eta = sqrt(((x - 3.0)**2)/(1.25)**2 + ((y - 3.0)**2)/(0.8)**2)
        eta = sqrt(((x )**2)/(1.1)**2 + ((y )**2)/(0.995)**2)
        rad = sqrt( x**2 + y**2)
        ...
        elsewhere
           ...
           Rp = R + 0.02*0.5*R*(3.0*cos(atan(y/x) )**2 - 1.0)

        endwhere
        !eta = ((x-2)**2 + (y-0.5)**2)	

        tmp = (half-minVF)  * ( one + tanh( (eta-(R))/(thick*dx) ) )
        !tmp = (half-minVF) * ( one + tanh( (rad - Rp)/(thick*dx)) )
      ! rad =  0.15825
...
!        offset = rank * nx
!        offset2 = rank *(nx+1)
!        mix%material(1)%p(:,:,1) = Fx(offset:offset2, :)
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Osc_Drop_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Osc_Drop_statistics.dat"
