module Multispecies_shock_data
    ...
    real(rkind) :: yield = one, yield2 = one, eta0k = 0.4_rkind, wave_mode = 0.0_rkind, wave_amp = 0.0_rkind
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    use SolidGrid,        only: rho_index,u_index,v_index,w_index
    ...
    real(rkind) :: stp_x,bkstp_x,bkstp_y,phiang,v1,v2,rad,stp_r1,bkstp_r2,regfrac,dxdy !u1,u2,
    real(rkind) :: uradinit = 1._rkind
    ...
    namelist /PROBINPUT/  p_infty, Rgas, gamma, mu, rho_0, p_amb, thick, minVF, &
                          ...
                          plastic2, explPlast2, yield2, wave_amp, wave_mode,&
    ...
    associate(   rho => fields(:,:,:,rho_index), u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        do k=1,decomp%ysz(3)
         do j=1,decomp%ysz(2)
          do i=1,decomp%ysz(1)
             ...
             u1 = -cos(phiang); u2 = zero
             v1 = -sin(phiang); v2 = zero
             
             rad = sqrt(x(i,j,k)**2+y(i,j,k)**2); dxdy = sqrt(dx**2+dy**2)

             !shock perturbation
             rad = rad + cos(phiang*wave_mode)*wave_amp
             
             stp_r1 = half*(tanh((rad-5.0d0)/(thick*dxdy)) + one)
             bkstp_r2 = one - half*(tanh((rad-15.0d0)/(thick*dxdy)) + one)
             regfrac = one - stp_r1*bkstp_r2

             u(i,j,k)   = uradinit * (u1+regfrac*(u2-u1))
             v(i,j,k)   = uradinit * (v1+regfrac*(v2-v1))
        ...
        tmp = mix%material(1)%g11*(mix%material(1)%g22*mix%material(1)%g33-mix%material(1)%g23*mix%material(1)%g32) - mix%material(1)%g12*(mix%material(1)%g21*mix%material(1)%g33-mix%material(1)%g31*mix%material(1)%g23) + mix%material(1)%g13*(mix%material(1)%g21*mix%material(1)%g32-mix%material(1)%g31*mix%material(1)%g22)

        ! Set thermodynamic varibles
        rho = rho_0 * tmp
        ...
        mix%material(1)%Ys = mix%material(1)%VF * rho_0
...
subroutine hook_bc(decomp,mesh,fields,mix,tsim,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
     ...
     if(decomp%yst(1)==decomp%xsz(1)) then !set top (y=yl) boundary
        ...
        mix%material(1)%g11( :,ny,:) = one;  mix%material(1)%g12( :,ny,:) = zero; mix%material(1)%g13( :,ny,:) = zero
        mix%material(1)%g21( :,ny,:) = zero; mix%material(1)%g22( :,ny,:) = one;  mix%material(1)%g23( :,ny,:) = zero
        mix%material(1)%g31( :,ny,:) = zero; mix%material(1)%g32( :,ny,:) = zero; mix%material(1)%g33( :,ny,:) = one

        mix%material(1)%gt11( :,ny,:) = one;  mix%material(1)%gt12( :,ny,:) = zero; mix%material(1)%gt13( :,ny,:) = zero
        mix%material(1)%gt21( :,ny,:) = zero; mix%material(1)%gt22( :,ny,:) = one;  mix%material(1)%gt23( :,ny,:) = zero
        mix%material(1)%gt31( :,ny,:) = zero; mix%material(1)%gt32( :,ny,:) = zero; mix%material(1)%gt33( :,ny,:) = one

        mix%material(1)%gp11( :,ny,:) = one;  mix%material(1)%gp12( :,ny,:) = zero; mix%material(1)%gp13( :,ny,:) = zero
        mix%material(1)%gp21( :,ny,:) = zero; mix%material(1)%gp22( :,ny,:) = one;  mix%material(1)%gp23( :,ny,:) = zero
        mix%material(1)%gp31( :,ny,:) = zero; mix%material(1)%gp32( :,ny,:) = zero; mix%material(1)%gp33( :,ny,:) = one
     endif
