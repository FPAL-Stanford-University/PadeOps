        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    ...
                    z(i,j,k) = real( iz1 - 1 + k - 1, rkind ) * dz  -2*pi
                end do
            end do
        end do
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz, x_bc,y_bc,z_bc)
    ...
    real(rkind) :: Lr, STRETCH_RATIO = 10.0
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w =>fields(:,:,:,w_index), uref =>fields(:,:,:,uref_index),rho => fields(:,:,:,rho_index),p=>fields(:,:,:,p_index), x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        eta = atanh(2.0*y /(1 + 1/STRETCH_RATIO))
        Lr    = 14.0/(eta(1,ny,1) - eta(1,1,1))
        eta = Lr*eta
    ...
    ! =========================================================================
        ...
        do q = 1, num_modes
          do l = 1, bnum_modes
            ...
            current_phase = 2.0_rkind * pi * sin( real(q)*1.37 + real(l)*3.81 )**2
            do k = 1, nz
                do j = 1, ny
                    do i = 1, nx
...
subroutine get_sponge(decomp,dx,dy,dz,mesh,fields,mix,rhou,rhov,rhow,rhoe,sponge)
    ...
    integer, allocatable :: data(:), recvbuf(:) 
        ...
        Lr    = 14.0/(yphys(1,ny,1) - yphys(1,1,1))
        yphys = Lr*yphys
      

        sigma1 = -5000 ! -2400 ! -80000

        where(yphys .LE. -6.0)
           sponge(:,:,:,1) = sigma1*( (yphys + 6.0)/1.0)**2.0
        elsewhere
           sponge(:,:,:,1) = 0
        endwhere

        where(yphys .GE. 6.0)
           sponge(:,:,:,2) = sigma1*( (yphys- 6.0)/1.0)**2.0 / 5
        elsewhere
           sponge(:,:,:,2) = 0
        endwhere
