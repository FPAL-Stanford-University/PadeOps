module bartonivp_data
    ...
    integer     :: ivpflag
    real(rkind) :: uL, uR, vL, vR, wL, wR, TL, TR, gL(3,3), gR(3,3), rhoL, rhoR, pL, pR
...
contains
  ...
  SUBROUTINE inv(a,inva)
    ...
    REAL(rkind), DIMENSION(:,:), INTENT(OUT) :: inva
    REAL(rkind) :: deta
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,eostype,eosparams,rho0,tstop,dt,tviz)
    ...
    real(rkind) :: finit(3,3)

    namelist /PROBINPUT/  pinit, thick, ivpflag
    ...
    associate( rho => fields(:,:,:,rho_index),   u => fields(:,:,:,  u_index), &
        ...
        if(ivpflag==1) then
          ...
          finit(3,1) = 0.00d0; finit(3,2) = 0.0d0; finit(3,3) = 1.0d0
          call inv(finit, gL)

          finit(1,1) = 1.00d0; finit(1,2) = 0.0d0; finit(1,3) = 0.0d0
          finit(2,1) = 0.00d0; finit(2,2) = 1.0d0; finit(2,3) = 0.1d0
          finit(3,1) = 0.00d0; finit(3,2) = 0.0d0; finit(3,3) = 1.0d0
          call inv(finit, gR)
        ...
        g11 = gL(1,1) + tmp * (gR(1,1) - gL(1,1));  
        g12 = gL(1,2) + tmp * (gR(1,2) - gL(1,2));  
        g13 = gL(1,3) + tmp * (gR(1,3) - gL(1,3));  
        g21 = gL(2,1) + tmp * (gR(2,1) - gL(2,1));  
        g22 = gL(2,2) + tmp * (gR(2,2) - gL(2,2));  
        g23 = gL(2,3) + tmp * (gR(2,3) - gL(2,3));  
        g31 = gL(3,1) + tmp * (gR(3,1) - gL(3,1));  
        g32 = gL(3,2) + tmp * (gR(3,2) - gL(3,2));  
        g33 = gL(3,3) + tmp * (gR(3,3) - gL(3,3));  
...
subroutine hook_output(decomp,der,fil,dx,dy,dz,outputdir,mesh,fields,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
        ...
        write(velstr,'(I3.3,a,I4.4)') ivpflag, '_', decomp%ysz(1)
        write(*,*) "/tec_bartonivp_"//trim(velstr)//".dat"
        write(outputfile,'(2A)') trim(outputdir),"/tec_bartonivp_"//trim(velstr)//".dat"
