module Noise_data
    ...
    real(rkind) :: Ly = 1.0, Lx = 0.2, interface_init = 0.75_rkind, shock_init = 0.6_rkind, kwave = 4.0_rkind
...
!        endif
        ...
        mix%material(1)%p  = p_amb !+ (noise-0.5)*1d-3   !66666dum + p1*(one-dum)
        mix%material(2)%p  = mix%material(1)%p

        mix%material(1)%VF = minVF +  (noise-0.5)*1d-5
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Noise_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Noise_statistics.dat"
