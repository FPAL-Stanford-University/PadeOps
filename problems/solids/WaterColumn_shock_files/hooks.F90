module WaterColumn_shock_data
    ...
    real(rkind) :: rhoRatio = one, pRatio = two, thick2
    logical     :: sharp = .FALSE.
    ...
    real(rkind) :: Ly = 10.0, Lx = 22.0, interface_init = 0.75_rkind, shock_init = 0.6_rkind, kwave = 4.0_rkind
...
subroutine initfields(decomp,der,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    associate(   rho    => fields(:,:,:, rho_index),  u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        shock_init = -1.0 !0.31 !interface_init !- 1.0_rkind  ! (10*thick) grid points away from the interface
        ...
        !tmp = (half)*(erf( (x-2.375_rkind + 1.0_rkind)/(thick*dx) ) - erf((x - 2.375_rkind - 1.0_rkind)/(thick*dx)))
        tmp = one - half * ( one - erf((0.25_rkind - (x-1.5_rkind)*(x-1.5_rkind) - (y-5.0_rkind)*(y-5.0_rkind))/(thick2*dx) ) )
        u   = 0.658*dum + (0.0)*(one-dum)
        v   = zero
        w   = zero
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,pthick,rhothick,uthick,Ysthick,VFthick,Ys_wiggle,VF_wiggle,x_bc,y_bc,z_bc)
    ...
    use SolidGrid,        only: rho_index,u_index,v_index,w_index,p_index,T_index,e_index,mu_index,bulk_index,kap_index, &
                                sxx_index,syy_index,szz_index,sxy_index,sxz_index,syz_index,sos_index, pEvolve_index
    use decomp_2d,        only: decomp_info, nrank
    use DerivativesMod,   only: derivatives
    use SolidMixtureMod,  only: solid_mixture
    ...
    real(rkind) :: vort_pos, vort_neg, mixwidth, Al_mass, xspike, xbubbl, xspike_proc, xbubbl_proc, ep, epp, pe, det_e, det_t, det_p, curl_e, curl_t, curl_p, rcount=0.0,eh,eel,ek,eh2,eel2,pe2, mom, mass
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
                 ...
                 sos  => fields(:,:,:,sos_index), pEvolve => fields(:,:,:,pEvolve_index ))
       ...
       do i = 1, decomp%ysz(1)
          do j = 1, decomp%ysz(2)
             do k = 1, decomp%ysz(3)
              ...
              mom = mom + mix%material(1)%VF(i,j,k)*mix%material(2)%rhom(i,j,k)*(u(i,j,k)*dx*dy + v(i,j,k)*dy*dx)
              mass = mass + mix%material(1)%VF(i,j,k)*mix%material(2)%rhom(i,j,k)*dy*dx 

             end do
           end do
       end do
       ! if (decomp%ysz(2) == 1) then
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/WaterColumn_shock_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/WaterColumn_shock_statistics.dat"

       
       if (vizcount == 0) then
           open(unit=outputunit, file=trim(outputfile), form='FORMATTED',status='REPLACE')
           write(outputunit,'(22A27)') 'tsim', 'momentum', 'mass'
       else
           open(unit=outputunit, file=trim(outputfile), form='FORMATTED',action='WRITE', status='OLD', position='APPEND')
       end if
       !write(outputunit,'(7ES27.16E3)') tsim, mixwidth, vort_pos, vort_neg,
       !Al_mass, xspike, xbubbl
       !write(outputunit,'(10ES27.16E3)') tsim, mixwidth, vort_pos, vort_neg,
       !Al_mass, xspike, xbubbl, ep, epp, pe
       if (nrank.eq.0) then
          !write(outputunit,'(12ES27.16E3)') real(nrank,rkind),rcount,tsim,
          !mixwidth, vort_pos, vort_neg, Al_mass, xspike, xbubbl, ep, epp, pe
          !write(outputunit,'(10ES27.16E3)') tsim, mixwidth, vort_pos, vort_neg,
          !Al_mass, xspike, xbubbl, ep, epp, pe
          write(outputunit,'(22F40.24)') tsim, mom, mass
       endif
       rcount = rcount + 1.0
       close(outputunit)
...
subroutine hook_bc(decomp,mesh,fields,mix,tsim,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
         v    => fields(:,:,:,   v_index), w   => fields(:,:,:,  w_index), pEvolve => fields(:,:,:,pEvolve_index), &
         p    => fields(:,:,:,   p_index), T   => fields(:,:,:,  T_index), &
         e    => fields(:,:,:,   e_index), mu  => fields(:,:,:, mu_index), &
         bulk => fields(:,:,:,bulk_index), kap => fields(:,:,:,kap_index), &
         x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
...
!         end do
              ...
              ! mix%material(1)%g11( 1,:,:) = rho2/rho_0; mix%material(1)%g12(
              ! 1,:,:) = zero; mix%material(1)%g13( 1,:,:) = zero
              ! mix%material(1)%g21( 1,:,:) = zero; mix%material(1)%g22( 1,:,:)
              ! = one;  mix%material(1)%g23( 1,:,:) = zero
              ! mix%material(1)%g31( 1,:,:) = zero; mix%material(1)%g32( 1,:,:)
              ! = zero; mix%material(1)%g33( 1,:,:) = one

              ! mix%material(2)%g11( 1,:,:) = rho2/rho_0;  mix%material(2)%g12(
              ! 1,:,:) = zero; mix%material(2)%g13( 1,:,:) = zero
              ! mix%material(2)%g21( 1,:,:) = zero; mix%material(2)%g22( 1,:,:)
