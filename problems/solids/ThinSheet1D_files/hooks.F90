module ThinSheet1D_data
    ...
    real(rkind) :: melt_t = one, melt_c = one, melt_t2 = one, melt_c2 = one,mu_ratio,l,Mdelta
    ...
    real(rkind) :: Ly = 10.0, Lx = 1,interface_init = 10d-3, kwave = 4.0_rkind, ksize = 10d0, etasize = 0.5d0, delta_d = 0.0125D0, delta = 0.0125D0, delta_rho = 0.0125D0 
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    ...
                    y(i,j,k) = real( iy1 - 1  + j - 1, rkind ) * dy - 5.0
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz, x_bc,y_bc,z_bc)
    ...
    integer :: ioUnit,i,iy
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp, dum, eta, eta2, yphys, u_perturb, KE, m1,m2,s,t,uin
    ...
    real(rkind) :: a0, a0_2,dx1,aa0,aa2,bb0,bb1,bb2,deltal,factor
...
!   dx1 = 2*pi/128
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        delta_rho = Nrho * dy * 0.275d0 !converts from Nrho to approximate thickness of erf profile
        ...
        uin = aa0 + aa2 * y**2.0_rkind
        where(abs(y) .LE. 1.0_rkind) 
              ! inside liquid sheet
              u = bb0 

       elsewhere(y .GT. 1.0_rkind .and. y .LE. (1.0_rkind + l)) 
              ! upper gas layer
              s = (y - 1.0_rkind) / l
              u = bb0 + bb1 * s + bb2 * s**2.0_rkind

       elsewhere(y .LT. -1.0_rkind .and. y .GE. -(1.0_rkind + l))
              ! lower gas layer
              s = (y + 1.0_rkind) / l
              u = bb0 - bb1 * s + bb2 * s**2.0_rkind
       ...
       t = (abs(y) - 1.0_rkind) / Mdelta;
       where(t .LE. 0)
           w = 0.0_rkind
       elsewhere( t .GE. 1 )
           w = 1.0_rkind
       elsewhere
           w = (6.0_rkind*t**5.0_rkind - 15.0_rkind*t**4.0_rkind + 10.0_rkind* t**3.0_rkind)
       endwhere

       u = u*w + (1.0_rkind-w)*uin
        ...
        tmp = 0.5_rkind*( erf( (y+1.0_rkind)/delta_rho ) - erf( (y-1.0_rkind)/delta_rho ) )
...
!       Lr    = 16.0/(yphys(1,ny,1) - yphys(1,1,1))
        ...
        where(yphys .GE. 3.5)
           sponge(:,:,:,2) = sigma1*( (yphys- 3.5)/1.5)**4.0 
        elsewhere
           sponge(:,:,:,2) = 0
        endwhere
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,pthick,rhothick,uthick,Ysthick,VFthick,Ys_wiggle,VF_wiggle,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/ThinSheet1D_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/ThinSheet1D_statistics.dat"
...
subroutine hook_bc(decomp,mesh,fields,mix,tsim,x_bc,y_bc,z_bc)
    ...
    integer :: nx,ny, i, j
    real(rkind) :: dy, yspng, tspng, yspngR, yspngL, Lr, STRETCH_RATIO = 2.5 
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
        ...
        if(decomp%yst(2)==1) then
          if(y_bc(1)==0) then
              ...
              v  ( :,1,:) = vL !v(:,2,:)
              w  ( :,1,:) = zero
              mix%material(1)%p(:,1,:) =  p_amb !mix%material(1)%p(:,2,:)
              mix%material(2)%p(:,1,:) =  p_amb !mix%material(2)%p(:,2,:)
        ...
        if(decomp%yen(2)==decomp%ysz(2)) then
          if(y_bc(2)==0) then
              ...
              v  ( :,ny,:) = vR !v(:,ny-1,:)
              w  ( :,ny,:) = zero
              mix%material(1)%p(:,ny,:) =  p_amb !mix%material(1)%p(:,ny-1,:)
              mix%material(2)%p(:,ny,:) =  p_amb !mix%material(2)%p(:,ny-1,:)
