module TwoBubble_data
    ...
    real(rkind) :: Ly = 1.5, Lx = 1.5, interface_init = 0.75_rkind, shock_init = 0.6_rkind, kwave = 4.0_rkind
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1  + i - 1, rkind ) * dx - 0.75!- two  ! x \in (-2,4]
                    y(i,j,k) = real( iy1  + j - 1, rkind ) * dy - 0.75
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz, x_bc,y_bc,z_bc)
    ...
    integer :: ioUnit
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp, dum, noise, tmpeta,eta,noise2
    ...
    namelist /PROBINPUT/  p_infty, Rgas, gamma, mu, rho_0, p_amb, thick, minVF, rhoRatio, pRatio, &
                          ...
                          diff_c_ge,diff_c_ge_2,diff_c_gp,diff_c_gp_2,diff_c_gt,diff_c_gt_2,v0
...
!
        ...
        where( x .GT. 0_rkind )
           ...
           tmp = half* ( one - erf((0.20_rkind*0.20_rkind - (x-0.3)*(x-0.3) -(y)*(y))/(thick*dx))) 

        elsewhere

           tmp = half*(one- erf((0.125_rkind*0.125_rkind - (x+0.25)*(x+0.25) -(y)*(y))/(thick*dx) ) ) 
        ...
        mix%material(1)%VF = minVF +  tmp !(one-two*minVF)*tmp ! +  (noise-0.5)*1d-5
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/TwoBubble_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/TwoBubble_statistics.dat"
