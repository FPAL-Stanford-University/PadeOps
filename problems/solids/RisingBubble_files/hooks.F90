module RisingBubble_data
    ...
    real(rkind) :: rhoL, rhoR, YsL, YsR, VFL, VFR, vL, vR, uL, uR,pL,pR
    real(rkind) :: rhoT, rhoB, YsT, YsB, VFT, VFB, vT, vB, uT, uB,pT,pB
    ...
    real(rkind) :: Ly = 2.0, Lx = 1.0,interface_init = 10d-3, kwave = 4.0_rkind, ksize = 10d0, etasize = 0.5d0, delta_d = 0.0125D0, delta = 0.0125D0, delta_rho = 0.0125D0 
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz, x_bc,y_bc,z_bc)
    ...
    real(rkind) :: fac, Lr, STRETCH_RATIO = 5.0, int_KE,rho_loc
...
!   dx1 = 2*pi/128
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        eta2 = 0.25d0*0.25d0 - y*y - x*x !  y-eta0k*cos(2.0_rkind*kwave*pi*x) 
        ...
        tmp = (half ) * ( one - erf( (eta2)/(delta_rho) ) )
...
!       do i = 1,nx
!       do j = ny,1,-1
!          if ( mix%material(1)%VF(i,j,k) .GT. 0.5 ) then
!            rho_loc = rho_0
!          else
!            rho_loc = rho_0_2
!          endif
...
!            p(:,j,:) = p_amb
!          else
!            p(:,j,:) = p(:,j+1,:) + rho_loc * 0.98 * (y(:,j+1,:) - y(:,j,:))
!          endif
!       enddo
!        enddo
        p(:,ny,:) = p_amb
       do j = ny-1,1,-1
       p(:,j,:) = p(:,j+1,:) +0.5*( rho(:,j,:)+rho(:,j+1,:))*(y(:,j+1,:)-y(:,j,:))*0.98
       enddo
        mix%material(1)%p = p
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,pthick,rhothick,uthick,Ysthick,VFthick,Ys_wiggle,VF_wiggle,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/RisingBubble_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/RisingBubble_statistics.dat"
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
        ...
        if(decomp%yst(2)==1) then
          if(y_bc(1)==0) then
              ...
              v  ( :,1,:) = zero !v(:,2,:)
              w  ( :,1,:) = zero
              mix%material(1)%p(:,1,:) =  pL !mix%material(1)%p(:,2,:)
              mix%material(2)%p(:,1,:) =  pL !mix%material(2)%p(:,2,:)
              p(:,1,:)                 =  pL
        ...
        if(decomp%yen(2)==decomp%ysz(2)) then
          if(y_bc(2)==0) then
              ...
              v  ( :,ny,:) = zero !v(:,ny-1,:)
              w  ( :,ny,:) = zero
              mix%material(1)%p(:,ny,:) =  pR !mix%material(1)%p(:,ny-1,:)
              mix%material(2)%p(:,ny,:) =  pR !mix%material(2)%p(:,ny-1,:)
              p(:,ny,:)                 = pR
