module ShearLayerEntrain_data
    ...
    real(rkind) :: v0=zero, v0_2=zero, tau0=1d-14, tau0_2=1d-14, Nrho = 1, U0 = zero, m = 1, p_mu = 1, p_mu2 = 1, epsilonk = 0,R=0.2,Gam = 1
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1 - 1   + i - 1, rkind ) * dx -2*pi/2.0
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz, x_bc,y_bc,z_bc)
    ...
    integer :: ioUnit,i,iy
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp, dum, eta, eta2, yphys, u_perturb, KE, m1,m2,r1,r2
    ...
    namelist /PROBINPUT/  p_infty, Rgas, gamma, mu, rho_0, p_amb, thick, minVF,  &
                          ...
                          diff_c_ge,diff_c_ge_2,diff_c_gp,diff_c_gp_2,diff_c_gt,diff_c_gt_2,alpha,epsP, epsRho, Gam, R, &
...
!   dx1 = 2*pi/128
    ...
    associate(   u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        r1 = sqrt( (x-0.6_rkind)**2_rkind + (yphys+1_rkind)**2_rkind) 
        r2 = sqrt( (x+0.6_rkind)**2_rkind + (yphys+0.8_rkind)**2_rkind)
        ...
        where( r1 < dx*3 ) 
           u = u -Gam/(2_rkind*pi*R**2_rkind)*(yphys+1_rkind)
           v = v +Gam/(2_rkind*pi*R**2_rkind)*(x-0.6_rkind)
        elsewhere

           u = u -Gam/(2_rkind*pi*r1**2_rkind)*(1_rkind - exp( - (r1/R)**2_rkind ))*(yphys+1_rkind)
           v = v + Gam/(2_rkind*pi*r1**2_rkind)*(1_rkind - exp( - (r1/R)**2_rkind))*(x-0.6_rkind)

        endwhere

        where( r2 < dx*3 )

           u = u -Gam/(2_rkind*pi*R**2_rkind)*(yphys+0.8_rkind)
           v = v +Gam/(2_rkind*pi*R**2_rkind)*(x+0.6_rkind)

        elsewhere

           u = u -Gam/(2_rkind*pi*r2**2_rkind)*(1_rkind - exp( - (r2/R)**2_rkind ))*(yphys+0.8_rkind)
           v = v + Gam/(2_rkind*pi*r2**2_rkind)*(1_rkind - exp( - (r2/R)**2_rkind))*(x+0.6_rkind)
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,pthick,rhothick,uthick,Ysthick,VFthick,Ys_wiggle,VF_wiggle,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/ShearLayerEntrain_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/ShearLayerEntrain_statistics.dat"
