module Acoustic_data
    ...
    real(rkind) :: Ly = 5.0, Lx = 5.0, interface_init = 0.5, kwave = 4.0_rkind, ksize = 10d0, etasize = 0.5d0, delta_d = 0.0125D0, delta = 0.0125D0, delta_rho = 0.0125D0 
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1 - 1 + i - 1, rkind ) * dx - 2.5
                    y(i,j,k) = real( iy1 - 1 + j - 1, rkind ) * dy - 2.5
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
    ...
    namelist /PROBINPUT/  p_infty, Rgas, gamma, mu, rho_0, p_amb, thick, minVF,  &
                          ...
                          v0, v0_2, tau0, tau0_2, eta0k, ksize, etasize, p_mu, p_mu2, Nvel, p_disturb,width
...
!        tmp = half * ( one - erf((625.0_rkind/7921.0_rkind - (y-0.5)*(y-0.5))/(thick*dx) ) )
        ...
        rho = rho_0*tmp + rho_0_2*(1-tmp) +  0.001*exp(-(x-1.25)**2_rkind /(2.0_rkind*(5.0_rkind/50_rkind)**2_rkind ))/(a0_2**2) !mix%material(1)%VF + rho_0_2*mix%material(2)%VF
        mix%material(1)%Ys =  mix%material(1)%VF * rho_0 / rho
        mix%material(2)%Ys = one - mix%material(1)%Ys ! Enforce sum to unity

        u = v0 +  0.001*exp(-(x-1.25)**2_rkind /(2.0_rkind*(5.0_rkind/50_rkind)**2_rkind )) /(rho_0_2*a0_2)
        v = 0 !+ 1d-9*(noise-0.5)
        w = 0
...
!	mix%material(1)%p  =p_amb+10*exp(-((y-0.8)**2)/(2*(0.05)**2))
        ...
        p = p_amb + 0.001*exp(-(x-1.25)**2_rkind /(2.0_rkind*(5.0_rkind/50_rkind)**2_rkind ))
        mix%material(1)%p = p !+ 1d-9*(noise3-0.5)
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Acoustic_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Acoustic_statistics.dat"
