module CollidingDroplet_data
    ...
    logical     :: sharp = .FALSE.
    real(rkind) :: p1,p2,rho1,rho2,u1,u2,g11_1,g11_2,grho1,grho2,a1,a2,pten,pten2
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz, x_bc,y_bc,z_bc)
    ...
    integer :: ioUnit
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp, dum, noise, tmpeta,eta,noise2,tmp2,tmp3,tmp4
    ...
    namelist /PROBINPUT/  p_infty, Rgas, gamma, mu, rho_0, p_amb, thick, minVF, rhoRatio, pRatio, &
                          ...
                          diff_c_ge,diff_c_ge_2,diff_c_gp,diff_c_gp_2,diff_c_gt,diff_c_gt_2,v0,pten,pten2
...
!
        ...
        print *, "tmp"
        tmp = half * ( one - erf(((0.1_rkind)**2 - (x-0.3_rkind)*(x-0.3_rkind) -(y)*(y))/(thick*dx) ) )
        tmp2 = half * ( one - erf(((0.1_rkind)**2 - (x+0.3_rkind)*(x+0.3_rkind) -(y)*(y))/(thick*dx) ) ) 
         
        tmp3 = half * ( one - erf(((0.1_rkind+5*dx)**2 - (x-0.3_rkind)*(x-0.3_rkind) -(y)*(y))/(1.0*dx) ) )
        tmp4 = half * ( one - erf(((0.1_rkind+5*dx)**2 - (x+0.3_rkind)*(x+0.3_rkind) -(y)*(y))/(1.0*dx) ) )
        ...
        p = p_amb + pten*tmp + pten2*tmp2
        mix%material(1)%p  = p !p_amb ! + (noise-0.5)*1d-7   !66666dum + p1*(one-dum)
        mix%material(2)%p  = mix%material(1)%p

        mix%material(1)%VF = minVF + tmp + tmp2 ! +  (noise-0.5)*1d-5
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/CollidingDroplet_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/CollidingDroplet_statistics.dat"
