...
module Droplet_Shear_data
    ...
    real(rkind) :: Ly = one, Lx = one, interface_init = 0.5d0, kwave = 4.0_rkind, Nvel = 1d0, delta = 0.01d0, ksize = 10d0, etasize = 0.5d0, Tp = 4d0 
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (decomp%ysz(2) == 1) then
           write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Droplet_Shear_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/Droplet_Shear_statistics.dat"
...
subroutine hook_timestep(decomp,mesh,fields,mix,step,tsim)
    ...
    use SolidMixtureMod,  only: solid_mixture
    use constants,        only: zero,eps,half,one,two,pi,six,four,eight
	...
	!ELSE IF ((tsim .GE. 4) .AND. (tsim .LT. 8)) THEN
 	!u = -  ((sin(pi*x))**2)*(sin(2*pi*y))*(cos(pi*(tsim-4)/Tp))
        ...
        !w = 0
	!ELSE IF ((tsim .GE. 8) .AND. (tsim .LT. 12)) THEN
        !u = -  ((sin(pi*x))**2)*(sin(2*pi*y))*(cos(pi*(tsim-8)/4))
        !v =  ((sin(pi*y))**2)*(sin(2*pi*x))*(cos(pi*(tsim-8)/4))
        !w = 0
	!ELSE IF ((tsim .GE. 12) .AND. (tsim .LT. 16)) THEN
        !u = -  ((sin(pi*x))**2)*(sin(2*pi*y))*(cos(pi*(tsim-12)/4))
        !v =  ((sin(pi*y))**2)*(sin(2*pi*x))*(cos(pi*(tsim-12)/4))
        !w = 0
 	!ELSE
        !u = -  ((sin(pi*x))**2)*(sin(2*pi*y))*(cos(pi*(tsim-16)/4))
        !v =  ((sin(pi*y))**2)*(sin(2*pi*x))*(cos(pi*(tsim-16)/4))
        !w = 0
	!ENDIF
