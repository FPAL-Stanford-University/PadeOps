module Advection_data
   ...
   real(rkind) :: Ly = one, Lx = 0.125d0, interface_init = 0.01d0, kwave = 4.0_rkind, ksize = 10d0, etasize = 0.5d0, delta_d = 0.0125D0, delta = 0.0125D0, delta_rho = 0.0125D0 
...
subroutine initfields(decomp,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
	...
	eta =(y-interface_init-eta0k*sin(2*pi*y/kwave))
        ...
        tmp = half*(erf( (eta+0.25_rkind)/(thick*dy) ) - erf( (eta-0.25_rkind)/(thick*dy)))
	!set mixture Volume fraction
	mix%material(1)%VF = tmp+minVF
	...
	mix%material(1)%p  = p_amb
        ...
        rhoL = rho(1,decomp%ysz(2),1)
        rhoR = rho(decomp%ysz(1),1,1)
        vL = v(1,1,1)
        vR = v(decomp%ysz(1),1,1)
        YsL  = mix%material(1)%Ys(1,decomp%ysz(2),1)
        YsR  = mix%material(1)%Ys(decomp%ysz(1),1,1)
        VFL  = mix%material(1)%VF(1,decomp%ysz(2),1)
