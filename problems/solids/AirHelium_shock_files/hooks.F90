...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1     + i - 1, rkind ) * dx - 0.5 ! x \in (-2,4]
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/AirHelium_shock_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/AirHelium_shock_statistics.dat"
