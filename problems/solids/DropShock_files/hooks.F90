module DropShock_data
    ...
    real(rkind) :: Ly = 10d0, Lx = 22d0, interface_init = 0.75_rkind, shock_init = 0.6_rkind, kwave = 4.0_rkind,thick2 = four
...
subroutine meshgen(decomp, dx, dy, dz, mesh)
    ...
    associate( x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
        ...
        do k=1,size(mesh,3)
            do j=1,size(mesh,2)
                do i=1,size(mesh,1)
                    x(i,j,k) = real( ix1     + i - 1, rkind ) * dx -2.0d0 ! x \in (-2,4]
...
subroutine initfields(decomp,der,derStagg,interpMid,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz,periodicx,periodicy,periodicz,x_bc,y_bc,z_bc)
    ...
    real(rkind), dimension(decomp%ysz(1),decomp%ysz(2),decomp%ysz(3)) :: tmp, dum,rhom,rhom2,r_droplet
    ...
    associate(  rho => fields(:,:,:,rho_index), u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
                p => fields(:,:,:,p_index), x => mesh(:,:,:,1), y => mesh(:,:,:,2), z => mesh(:,:,:,3) )
...
!        endif
        ...
        shock_init = 0.0_rkind !interface_init - 1.0_rkind  ! (10*thick) grid points away from the interface
        dum = half * ( one - erf( (x)/(thick2*dx) ) )
        u2 = 0.658d0 !9473
        u1 = 0
        u   = (u2-u1)*dum
        v   = zero
        ...
        !tmp = half * ( one - erf((4d-3*4d-3 - (x-5d-3)*(x-5d-3) - (y-4d-2)*(y-4d-2))/(thick*dx) ) )
        ! Compute radial distance from droplet center
        r_droplet = sqrt((x-1.5_rkind)**2 + (y-5._rkind)**2)

        ! Interface at r = 0.004 m
        tmp = half * ( one - erf((r_droplet - 1_rkind)/(thick*dx)) )
        ...
        mix%material(1)%g11 = one ! (rho2*dum + rho1*(one-dum))/rho_0
  ...
  !      end if
        ...
        rhom2 = 1.18d0*dum + 1.0d0*(one-dum)
        rho = rhom2*(one-tmp) + 820.903d0*(tmp)
        !if (mix%use_gTg.and.(.not.mix%strainHard)) then
        ...
        !end if
        mix%material(1)%Ys = mix%material(1)%VF * rhom2 / rho
        mix%material(2)%Ys = one - mix%material(1)%Ys ! Enforce sum to unity

        rhoL = 1.18d0
        rhoR = 1.0d0 ! tmp(decomp%ysz(1),1,1)
     ...
     !  end if
        ...
        !rhoL = (rho_0*VFL + rho_0_2*(one-VFL))*rho1/rho_0
        !rhoR = (rho_0*VFR + rho_0_2*(one-VFR))*rho2/rho_0
        ...
        mix%material(2)%intSharp_ysc(1) = one-YsL
        mix%material(2)%intSharp_ysc(2) = one-YsR
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/DropShock_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/DropShock_statistics.dat"
...
!            ! call filter3D(decomp,mygfil,tmp,1,x_bc,y_bc,z_bc)
        ...
        if(decomp%yen(1)==decomp%xsz(1)) then
          if(x_bc(2)==0) then
            ...
            !v  (nx,:,:) =  v(nx-1,:,:)
            !w  (nx,:,:) = 0 ! w(nx-1,:,:)
            mix%material(1)%p  (nx,:,:) = p_amb ! mix%material(1)%p(nx-1,:,:)
            mix%material(2)%p  (nx,:,:) = p_amb !  mix%material(2)%p(nx-1,:,:)
            p(nx,:,:) = p_amb !(nx-1,:,:) 
            ...
            mix%material(1)%Ys (nx,:,:) = YsR   !mix%material(1)%Ys (nx-1,:,:)
            mix%material(2)%Ys (nx,:,:) = 1-YsR !mix%material(2)%Ys (nx-1,:,:)
            
            mix%material(1)%VF (nx,:,:) = VFR !mix%material(1)%VF (nx-1,:,:)
            mix%material(2)%VF (nx,:,:) =  1-VFR !mix%material(1)%VF (nx-1,:,:)
         endif
      endif
