module CavityCollapse_shock_data
    ...
    real(rkind) :: Ly = 5.0, Lx = 10.0, interface_init = 0.75_rkind, shock_init = 0.6_rkind, kwave = 4.0_rkind
...
subroutine initfields(decomp,der,dx,dy,dz,inputfile,mesh,fields,mix,tstop,dt,tviz)
    ...
    associate(   rho    => fields(:,:,:, rho_index),  u => fields(:,:,:,u_index), v => fields(:,:,:,v_index), w => fields(:,:,:,w_index), &
        ...
        tmp = one - half * ( one - erf((1.0_rkind - (x-2.375_rkind)*(x-2.375_rkind) - (y-2.5_rkind)*(y-2.5_rkind))/(thick2*dx) ) )
        u   = 68.5176*dum + (0.0)*(one-dum)
        v   = zero
        w   = zero
...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,pthick,rhothick,uthick,Ysthick,VFthick,Ys_wiggle,VF_wiggle,x_bc,y_bc,z_bc)
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       !     write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/CavityCollapse_shock_"//trim(str)//"_", vizcount, ".dat"
       ...
       write(outputfile,'(2A,I4.4,A)') trim(outputdir),"/CavityCollapse_shock_statistics.dat"
