...
contains
    ...
    subroutine write_post2d(step, vort_avgz, TKE_avg, div_avg, rho_avg, chi_avg, MMF_avg, CO2_avg, density_self_correlation, &
                            R11, R12, R13, R22, R23, R33, a_x, a_y, a_z, eta, xi)
        ...
        real(rkind), dimension(:,:),   intent(in) :: R11, R12, R13, R22, R23, R33, a_x, a_y, a_z, eta, xi
...
program IRM_vorticity
    ...
    real(rkind), dimension(:,:), pointer :: a_x, a_y, a_z, eta, xi
    ...
    allocate( buffer2d( mir%gp%ysz(1), mir%gp%ysz(2), 24 ) )
