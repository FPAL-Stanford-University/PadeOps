program EkmanLayerDNSMomentumBalance
    ...
    do while(idx <= nt)
        ...
        call ops%ddz(v-vfluct, buff2, botBC, topBC )
        call ops%TakeMean_xy(buff1, dUdz_t(:,idx))
        call ops%TakeMean_xy(buff2, dVdz_t(:,idx))
        ...
        call ops%ddz(ufluct*w, buff1, -1, -1)
        call ops%ddz(vfluct*w, buff2, -1, -1)
        call ops%TakeMean_xy(buff1, duwdz_t(:,idx))
        call ops%TakeMean_xy(buff2, dvwdz_t(:,idx))
    ...
    if (nrank == 0) then
       call ops%WriteASCII_2D(umean_t, "U___")
       call ops%WriteASCII_2D(vmean_t, "V___")
       call ops%WriteASCII_2D(dUdz_t, "dU__")
       call ops%WriteASCII_2D(dVdz_t, "dV__")
       call ops%WriteASCII_2D(d2Udz2_t, "ddU_")
       call ops%WriteASCII_2D(d2Vdz2_t, "ddV_")
       call ops%WriteASCII_2D(duwdz_t, "duw_")
       call ops%WriteASCII_2D(dvwdz_t, "dvw_")
       call ops%WriteASCII_2D(uw_t, "uw__")
       call ops%WriteASCII_2D(vw_t, "vw__")
    end if 
