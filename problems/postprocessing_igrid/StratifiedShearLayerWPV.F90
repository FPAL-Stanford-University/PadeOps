...
program StratifiedShearLayerWPV
   ...
   do while(idx <= nt)
      ...
      ! STEP 1: Compute Wave-PV Decomposition of Vorticity
      ...
      call ops%WriteField3D(buff7, "PVPi", tidx)

      buff8 = buff4*buff4 + buff5*buff5 + buff6*buff6
      buff8 = buff8**(1.d0/2.d0) ! |gradT|

      buff7 = buff7/buff8 ! |omega_Pi|
      call ops%WriteField3D(buff7, "OmPi", tidx)

      buff1 = buff1 - buff7*buff4/buff8
      buff2 = buff2 - buff7*buff5/buff8
      buff3 = buff3 - buff7*buff6/buff8

      buff1 = buff1*buff1 + buff2*buff2 + buff3*buff3
      buff1 = buff1**(1.d0/2.d0) ! |omega_Xi|
      call ops%WriteField3D(buff1, "OmXi", tidx)

      buff1 = buff1 * buff8 ! Xi
      call ops%WriteField3D(buff1, "PVXi", tidx)


      ! STEP 2: Compute Wave-PV Decomposition of Velocity

      call ComputeF(ops,ufluct,vfluct,w,T,buff1,buff2,buff3,buff4,buff5,buff6,buff7)

      ! TEMPORARY
      call ops%WriteField3D(buff1, "PVFx", tidx)
      call ops%WriteField3D(buff2, "PVFy", tidx)
      call ops%WriteField3D(buff3, "PVFz", tidx)
