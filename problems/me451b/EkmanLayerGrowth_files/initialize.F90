...
subroutine meshgen_wallM(decomp, dx, dy, dz, mesh, inputfile)
    ...
    namelist /EkmanLayerGrowth/ Lx, Ly, Lz, alphaRot 
...
subroutine initfields_wallM(decompC, decompE, inputfile, mesh, fieldsC, fieldsE)
    ...
    integer :: nz, nzE, k
    real(rkind)  :: Lx = one, Ly = one, Lz = 20.d0
    real(rkind) :: periods = 2.d0, epsnd = 0.5, zpeak = 1.d0
    namelist /EkmanLayerGrowth/ Lx, Ly, Lz, alphaRot 
    ...
    v = v - epsnd*(z/Lz)*sin(periods*2*pi*x/Lx)*cos(periods*2*pi*y/Lx)*exp(-0.5*(z/zpeak/Lz)**2) 
    !wC= zero  
