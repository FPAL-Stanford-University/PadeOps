program test_Pade6ops
    ...
    real(rkind), dimension(:,:,:), allocatable :: uC, zC, zE, uE, d2udz2C, dudzE
    type(decomp_info) :: gpC
    type(decomp_info), pointer :: sp_gpC
    type(spectral), target :: spectC
    integer :: nx = 16, ny = 16, nz = 32
    integer :: ierr, k 
    real(rkind) :: dx, dy, dz
