program test_derivatives2_parallel
    use mpi
    ...
    real(rkind), dimension(:,:,:), allocatable :: x, y, z, f,d2fdx2, d2fdy2, d2fdz2, df
    real(rkind), dimension(:,:,:), allocatable :: f_in_x, df_in_x, f_in_z, df_in_z
    type(derivatives) :: method1
    type(decomp_info) :: gp
    integer :: nx = 256, ny = 256, nz = 512
    ...
    call method1 % d2dx2(f_in_x,df_in_x)
    call transpose_x_to_y(df_in_x,df,gp)
    t1 = MPI_WTIME()
    mymaxerr = MAXVAL(ABS(df - d2fdx2))
    ...
    call method1 % d2dz2(f_in_z,df_in_z)
    call transpose_z_to_y(df_in_z,df,gp)
    t1 = MPI_WTIME()
    mymaxerr = MAXVAL(ABS(df - d2fdz2))
    call MPI_Reduce(mymaxerr, maxerr, 1, real_type, MPI_MAX, 0, MPI_COMM_WORLD, ierr)
    if (nrank == 0) print*, "Maximum error = ", maxerr
    ...
    deallocate(x, y, z, f, d2fdx2,d2fdy2, d2fdz2)
