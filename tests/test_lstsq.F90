program test_lstsq
    ...
    ierr = xlstsq%init( nx, periodic)
    ierr = ylstsq%init( ny, periodic)
    ierr = zlstsq%init( nz, periodic)

    call tic() 
    call xlstsq % filter1(f,df,ny,nz)
    call toc ("Time to get the x filter:")
    print*, "Maximum error = ", MAXVAL( ABS(df - df_x))

    call tic() 
    call ylstsq % filter2(f,df,nx,nz)
    call toc ("Time to get the y filter:")
    print*, "Maximum error = ", MAXVAL( ABS(df - df_y))

    OPEN(UNIT=iounit, FILE="lstsq.txt", FORM='FORMATTED')
    do i=1,ny
    ...
    call tic() 
    call zlstsq % filter3(f,df,nx,ny)
