program test_HITforcing
   ...
   integer :: i, n, Ninputs = 1, Nrealizations = 10000
   ...
   call HITForce%init("/anvil/projects/x-atm170028/ryanhass/HITforced"//&
     "/HITforcingDebug/HIT128_DNS2_InputData/HIT_forcing_input_test.dat", spectC%spectdecomp, &
   ...
   do n = 1,Ninputs 
     ...
     do i = 1,Nrealizations
       ...
       if (mod(i,dumpFreq) == 0) then
         write(fname,'(A,I5.5,A,I6.6,A)')trim(outputdir)//'/u_HITforcing_128_',i,'_t',tid,'.dat'
         call decomp_2d_write_one(1,u ,trim(fname), gp)
         write(fname,'(A,I5.5,A,I6.6,A)')trim(outputdir)//'/v_HITforcing_128_',i,'_t',tid,'.dat'
         call decomp_2d_write_one(1,v ,trim(fname), gp)
         write(fname,'(A,I5.5,A,I6.6,A)')trim(outputdir)//'/w_HITforcing_128_',i,'_t',tid,'.dat'
