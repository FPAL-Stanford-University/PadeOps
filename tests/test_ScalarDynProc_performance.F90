program test_ScalarDynProc_performance
   ...
   real(rkind),    dimension(:,:,:), allocatable :: fbody_x, fbody_y, fbody_z, dTdxC, dTdyC, dTdzC, dTdzE, nuSGSTrue, kappaSGSTrue
   ...
   allocate(nuSGStrue (gp %xsz(1), gp %xsz(2), gp %xsz(3)))
   allocate(kappaSGStrue (gp %xsz(1), gp %xsz(2), gp %xsz(3)))
   ...
   call decomp_2d_read_one(1,nuSGStrue,'/work/04076/tg833754/stampede2/me461/testing_data/nuSGS_verify.out',gp)
   call decomp_2d_read_one(1,kappaSGStrue,'/work/04076/tg833754/stampede2/me461/testing_data/kappaSGS_verify.out',gp)

   maxErr_kappa = p_maxval(maxval(abs(kappaSGS - kappaSGStrue)))
   maxErr_nu    = p_maxval(maxval(abs(nuSGS    - nuSGStrue)))

   
   call message("=================================================")
   call message("=================================================")
   call message(0," Code accuracy summary:")
   call message(1, "Max error in nu_SGS", maxErr_nu) 
   if (maxErr_nu > 1.d-13) then
      call message(1,"WARNING: Looks like you might have changed the code for dynamic procedure for momentum.")
   end if
   call message(1, "Max error in kp_SGS", maxErr_kappa) 
   if (maxErr_kappa > 1.d-13) then
      call message(0,"Test failed. Keep debugging...")
   else
      call message(0,"Implementation is accurate.")
   end if
   call mpi_barrier(mpi_comm_world, ierr)

   call message(0,"Code performance summary:")
   call message(1, "Time taken to compute kappa_sgs:", tactual)
   call message(1,"Estimate for code performance (Stampede 2, 1 KNL node, 64 cores)", (tactual/texpect)*100.d0)
